...
use panoptes_cli_output::{CliEpochVotingHistory, CliLockout, CliVoteAccount};
            ...
            SubCommand::with_name("withdraw-from-vote-account")
                ...
                .arg(
                    pubkey!(Arg::with_name("destination_account_pubkey")
                        ...
                        .required(true),
                        "The recipient of withdrawn PANO. "),
                )
                .arg(
                    Arg::with_name("amount")
                        ...
                        .help("The amount to withdraw, in PANO; accepts keyword ALL"),
                )
                .arg(
                    Arg::with_name("authorized_withdrawer")
                        .long("authorized-withdrawer")
...
pub fn process_create_vote_account(
    ...
    let vote_account_address = if let Some(seed) = seed {
        Pubkey::create_with_seed(&vote_account_pubkey, seed, &panoptes_vote_program::id())?
    } else {
        vote_account_pubkey
    };
        ...
        rpc_client.get_account_with_commitment(&vote_account_address, config.commitment)
    {
        if let Some(vote_account) = response.value {
            let err_msg = if vote_account.owner == panoptes_vote_program::id() {
        ...
        SpendAmount::Some(withdraw_amount) => {
            if current_balance.saturating_sub(withdraw_amount) < minimum_balance {
                return Err(CliError::BadParameter(format!(
                    "Withdraw amount too large. The vote account balance must be at least {} PANO to remain rent exempt", lamports_to_sol(minimum_balance)
                ))
                .into());
            }
            withdraw_amount
...
mod tests {
    ...
    use panoptes_sdk::signature::{read_keypair_file, write_keypair, Keypair, Signer};
    ...
    fn test_parse_command() {
        ...
        let authed = panoptes_sdk::pubkey::new_rand();
        let (keypair_file, mut tmp_file) = make_tmp_file();
        let keypair = Keypair::new();
        write_keypair(&keypair, tmp_file.as_file_mut()).unwrap();
