...
use panoptes_cli::cli::{
    app, parse_command, process_command, CliCommandInfo, CliConfig, SettingType,
    DEFAULT_RPC_TIMEOUT_SECONDS,
};
use panoptes_cli_config::{Config, CONFIG_FILE};
use panoptes_cli_output::{display::println_name_value, OutputFormat};
...
fn main() -> Result<(), Box<dyn error::Error>> {
    panoptes_logger::setup_with_default("off");
    let matches = app(
        crate_name!(),
        crate_description!(),
    ...
    .arg(
        Arg::with_name("websocket_url")
            ...
            .global(true)
            .validator(is_url)
            .help("WebSocket URL for the panoptes cluster"),
    )
    .arg(
    ...
    .subcommand(
        SubCommand::with_name("config")
            .about("Panoptes command-line tool configuration settings")
            .aliases(&["get", "set"])
