...
use panoptes_account_decoder::{UiAccount, UiAccountEncoding};
...
use panoptes_transaction_status::{EncodedTransaction, UiTransactionEncoding};
use panoptes_vote_program::vote_state::VoteAuthorize;
use std::{
...
impl CliConfig<'_> {
    fn default_keypair_path() -> String {
        panoptes_cli_config::Config::default().keypair_path
    }

    fn default_json_rpc_url() -> String {
        panoptes_cli_config::Config::default().json_rpc_url
    }

    fn default_websocket_url() -> String {
        panoptes_cli_config::Config::default().websocket_url
    }
    ...
    pub fn compute_websocket_url_setting(
        ...
        Self::first_nonempty_setting(vec![
            ...
            (
                ...
                panoptes_cli_config::Config::compute_websocket_url(&normalize_to_url_if_moniker(
                    json_rpc_cmd_url,
                )),
            ),
            (
...
fn resolve_derived_address_program_id(matches: &ArgMatches<'_>, arg_name: &str) -> Option<Pubkey> {
    matches.value_of(arg_name).and_then(|v| match v {
        ...
        "VOTE" => Some(panoptes_vote_program::id()),
...
fn process_airdrop(
    ...
    if let Ok(signature) = result {
        ...
        if current_balance < pre_balance.saturating_add(lamports) {
            ...
            println!("Run `panoptes confirm -v {:?}` for more info", signature);
            Ok("".to_string())
        } else {
...
pub fn process_command(config: &CliConfig) -> ProcessResult {
    ...
    match &config.command {
        ...
        // Return software version of panoptes-cli and cluster entrypoint node
        CliCommand::Catchup {
            node_pubkey,
            node_json_rpc_url,
            follow,
        ...
        // Request an airdrop from Panoptes Faucet;
            ...
            SubCommand::with_name("airdrop")
                ...
                .arg(
                    Arg::with_name("faucet_port")
                        ...
                        .takes_value(true)
                        .default_value(panoptes_faucet::faucet::FAUCET_PORT_STR)
...
mod tests {
    ...
    use panoptes_transaction_status::TransactionConfirmationStatus;
    ...
    fn test_generate_unique_signers() {
        ...
        assert_eq!(
            signer_info.index_of(Some(panoptes_sdk::pubkey::new_rand())),
            None
        );
    ...
    fn test_cli_parse_command() {
        ...
        let from_pubkey = Some(panoptes_sdk::pubkey::new_rand());
        let from_str = from_pubkey.unwrap().to_string();
        for (name, program_id) in &[
            ("STAKE", stake::program::id()),
            ("VOTE", panoptes_vote_program::id()),
    ...
    fn test_cli_process_command() {
        ...
        let new_authorized_pubkey = panoptes_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = panoptes_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = panoptes_sdk::pubkey::new_rand();
        let source_stake_account_pubkey = panoptes_sdk::pubkey::new_rand();
