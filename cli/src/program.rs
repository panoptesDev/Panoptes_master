...
use log::*;
use panoptes_account_decoder::{UiAccountEncoding, UiDataSliceConfig};
use panoptes_bpf_loader_program::{bpf_verifier, BpfError, ThisInstructionMeter};
use panoptes_clap_utils::{self, input_parsers::*, input_validators::*, keypair::*};
...
use panoptes_rbpf::vm::{Config, Executable};
...
fn report_ephemeral_mnemonic(words: usize, mnemonic: bip39::Mnemonic) {
    ...
    eprintln!(
        "`panoptes-keygen recover` and the following {}-word seed phrase:",
        words
    );
    eprintln!("{}\n{}\n{}", divider, phrase, divider);
    eprintln!("To resume a deploy, pass the recovered keypair as");
    eprintln!("the [PROGRAM_ADDRESS_SIGNER] argument to `panoptes deploy` or");
    eprintln!("as the [BUFFER_SIGNER] to `panoptes program deploy` or `panoptes write-buffer'.");
    eprintln!("Or to recover the account's lamports, pass it as the");
    eprintln!(
        "[BUFFER_ACCOUNT_ADDRESS] argument to `panoptes program close`.\n{}",
        divider
    );
}
...
mod tests {
    ...
    use panoptes_cli_output::OutputFormat;
    use panoptes_sdk::signature::write_keypair_file;
