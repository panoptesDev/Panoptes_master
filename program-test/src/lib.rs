//! The panoptes-program-test provides a BanksClient-based test framework BPF programs
...
use {
    ...
    log::*,
    panoptes_banks_client::start_client,
    panoptes_banks_server::banks_server::start_local_server,
    panoptes_runtime::{
...
// Export types so test clients can limit their panoptes crate dependencies
pub use panoptes_banks_client::BanksClient;
...
pub fn builtin_process_instruction(
    process_instruction: panoptes_sdk::entrypoint::ProcessInstruction,
    program_id: &Pubkey,
    input: &[u8],
...
/// Converts a `panoptes-program`-style entrypoint into the runtime's entrypoint style, for
...
macro_rules! processor {
    ($process_instruction:expr) => {
        Some(
            |program_id: &Pubkey,
             input: &[u8],
             invoke_context: &mut dyn panoptes_sdk::process_instruction::InvokeContext| {
...
struct SyscallStubs {}
impl panoptes_sdk::program_stubs::SyscallStubs for SyscallStubs {
    ...
    fn sol_invoke_signed(
        ...
        panoptes_runtime::message_processor::MessageProcessor::process_cross_program_instruction(
            &message,
            &executables,
            &accounts,
            &caller_privileges,
...
impl Default for ProgramTest {
    ...
    ///
    fn default() -> Self {
        panoptes_logger::setup_with_default(
            "panoptes_rbpf::vm=debug,\
             panoptes_runtime::message_processor=debug,\
             panoptes_runtime::system_instruction_processor=trace,\
             panoptes_program_test=info",
        );
...
impl ProgramTest {
        ...
        let add_bpf = |this: &mut ProgramTest, program_file: PathBuf| {
            ...
            this.add_account(
                ...
                Account {
                    ...
                    data,
                    owner: panoptes_sdk::bpf_loader::id(),
                    executable: true,
                    rent_epoch: 0,
                },
            );
    ...
    fn setup_bank(
    ...
    ) {
        {
            ...
            ONCE.call_once(|| {
                panoptes_sdk::program_stubs::set_syscall_stubs(Box::new(SyscallStubs {}));
            });
        }
        ...
        add_builtin!(panoptes_bpf_loader_deprecated_program!());
        if self.use_bpf_jit {
            add_builtin!(panoptes_bpf_loader_program_with_jit!());
            add_builtin!(panoptes_bpf_loader_upgradeable_program_with_jit!());
        } else {
            add_builtin!(panoptes_bpf_loader_program!());
            add_builtin!(panoptes_bpf_loader_upgradeable_program!());
        }
    ...
    /// with PANO for sending transactions
...
impl ProgramTestContext {
    ...
    pub fn warp_to_slot(&mut self, warp_slot: Slot) -> Result<(), ProgramTestError> {
        ...
        bank_forks.set_root(
            pre_warp_slot,
            &panoptes_runtime::accounts_background_service::AbsRequestSender::default(),
            Some(warp_slot),
        );
