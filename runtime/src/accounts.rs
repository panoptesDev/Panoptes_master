...
impl Accounts {
        ...
        if tx.signatures.is_empty() && fee != 0 {
            ...
            for (i, key) in message.account_keys.iter().enumerate() {
                let account = if key_check.is_non_loader_key(key, i) {
                    ...
                    if panoptes_sdk::sysvar::instructions::check_id(key)
    ...
    pub fn lock_accounts<'a>(
        ...
        use panoptes_sdk::sanitize::Sanitize;
...
mod tests {
    ...
    fn test_load_accounts_invalid_account_for_fee() {
        ...
        let account = AccountSharedData::new(1, 1, &panoptes_sdk::pubkey::new_rand()); // <-- owner is not the system program
        accounts.push((key0, account));
    ...
    fn test_instructions() {
        ...
        let instructions_key = panoptes_sdk::sysvar::instructions::id();
        let keypair = Keypair::new();
        let instructions = vec![CompiledInstruction::new(1, &(), vec![0, 1])];
        let tx = Transaction::new_with_compiled_instructions(
            &[&keypair],
            &[panoptes_sdk::pubkey::new_rand(), instructions_key],
            Hash::default(),
            vec![native_loader::id()],
            instructions,
        );
