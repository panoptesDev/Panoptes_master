...
use panoptes_sdk::{feature_set, pubkey::Pubkey};
...
fn genesis_builtins(bpf_jit: bool) -> Vec<Builtin> {
    vec![
        to_builtin!(panoptes_bpf_loader_deprecated_program!()),
        if bpf_jit {
            to_builtin!(panoptes_bpf_loader_program_with_jit!())
        } else {
            to_builtin!(panoptes_bpf_loader_program!())
        },
    ]
}
...
fn feature_builtins(bpf_jit: bool) -> Vec<(Builtin, Pubkey, ActivationType)> {
    vec![(
        if bpf_jit {
            to_builtin!(panoptes_bpf_loader_upgradeable_program_with_jit!())
        } else {
            to_builtin!(panoptes_bpf_loader_upgradeable_program!())
        },
