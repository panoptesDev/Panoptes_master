//! The `rpc_service` module implements the Panoptes JSON RPC service.
...
use {
    ...
    regex::Regex,
    panoptes_client::rpc_cache::LargestAccountsCache,
    panoptes_gossip::cluster_info::ClusterInfo,
    panoptes_ledger::{
        bigtable_upload_service::BigTableUploadService, blockstore::Blockstore,
        leader_schedule_cache::LeaderScheduleCache,
    },
    panoptes_metrics::inc_new_counter_info,
    panoptes_poh::poh_recorder::PohRecorder,
    panoptes_runtime::{
    ...
    match path {
        "/v0/circulating-supply" => {
            ...
            let non_circulating_supply =
                panoptes_runtime::non_circulating_supply::calculate_non_circulating_supply(&bank)
    ...
    pub fn new(
        ...
        let (bigtable_ledger_storage, _bigtable_ledger_upload_service) =
            if config.enable_bigtable_ledger_storage || config.enable_bigtable_ledger_upload {
                runtime
                    .block_on(panoptes_storage_bigtable::LedgerStorage::new(
        ...
        let thread_hdl = Builder::new()
            .name("panoptes-jsonrpc".to_string())
            .spawn(move || {
...
mod tests {
    use {
        ...
        panoptes_gossip::crds_value::{CrdsData, CrdsValue, SnapshotHash},
        panoptes_ledger::{
    ...
    fn test_rpc_new() {
        ...
        let rpc_addr = SocketAddr::new(
            ip_addr,
            panoptes_net_utils::find_available_port_in_range(ip_addr, (10000, 65535)).unwrap(),
        );
        ...
        assert_eq!(thread.name().unwrap(), "panoptes-jsonrpc");
