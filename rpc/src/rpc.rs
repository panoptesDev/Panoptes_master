//! The `rpc` module implements the Panoptes RPC interface.
...
use {
    ...
    panoptes_faucet::faucet::request_airdrop_transaction,
    panoptes_gossip::{cluster_info::ClusterInfo, contact_info::ContactInfo},
    panoptes_ledger::{
        blockstore::Blockstore, blockstore_db::BlockstoreError, get_tmp_ledger_path,
        leader_schedule_cache::LeaderScheduleCache,
    },
    panoptes_metrics::inc_new_counter_info,
    panoptes_perf::packet::PACKET_DATA_SIZE,
    panoptes_runtime::{
    ...
    panoptes_vote_program::vote_state::{VoteState, MAX_LOCKOUT_HISTORY},
    spl_token_v2_0::{
        panoptes_program::program_pack::Pack,
...
pub struct JsonRpcRequestProcessor {
    ...
    runtime: Arc<Runtime>,
    bigtable_ledger_storage: Option<panoptes_storage_bigtable::LedgerStorage>,
...
impl JsonRpcRequestProcessor {
    ...
    fn bank(&self, commitment: Option<CommitmentConfig>) -> Arc<Bank> {
        ...
        r_bank_forks.get(slot).cloned().unwrap_or_else(|| {
            ...
            // a slot.
            //
            // For more information, see https://github.com/panoptes-labs/panoptes/issues/11078
            warn!(
    ...
    fn check_bigtable_result<T>(
        &self,
        result: &std::result::Result<T, panoptes_storage_bigtable::Error>,
    ) -> Result<()>
    where
        T: std::fmt::Debug,
    {
        if result.is_err() {
            let err = result.as_ref().unwrap_err();
            if let panoptes_storage_bigtable::Error::BlockNotFound(slot) = err {
    ...
    pub fn get_confirmed_signatures_for_address(
        ...
        if self.config.enable_rpc_transaction_history {
            ...
            // https://github.com/panoptes-labs/panoptes/pull/10928
            let end_slot = min(
                end_slot,
    ...
    pub fn get_stake_activation(
        ...
        if bank.epoch().saturating_sub(epoch) > panoptes_sdk::stake_history::MAX_ENTRIES as u64 {
            return Err(Error::invalid_params(format!(
        ...
        let stake_history =
            panoptes_sdk::account::from_account::<StakeHistory, _>(&stake_history_account)
                .ok_or_else(Error::internal_error)?;
...
pub mod rpc_minimal {
    ...
    pub trait Minimal {
        ...
        // TODO: Refactor `panoptes-validator wait-for-restart-window` to not require this method, so
    ...
    impl Minimal for MinimalImpl {
        ...
        fn get_version(&self, _: Self::Metadata) -> Result<RpcVersionInfo> {
            ...
            let version = panoptes_version::Version::default();
            Ok(RpcVersionInfo {
                panoptes_core: version.to_string(),
            ...
            Ok(meta
                ...
                .map(|leader_schedule| {
                    let mut schedule_by_identity =
                        panoptes_ledger::leader_schedule_utils::leader_schedule_by_identity(
...
pub mod rpc_full {
    ...
    pub trait Full {
        ...
        // See https://github.com/panoptes-labs/panoptes-program-library/releases/tag/token-v2.0.0 for
        // program details
    ...
    impl Full for FullImpl {
        ...
        fn send_transaction(
            ...
            let durable_nonce_info = panoptes_sdk::transaction::uses_durable_nonce(&transaction)
                .and_then(|nonce_ix| {
                    panoptes_sdk::transaction::get_nonce_pubkey_from_instruction(
                        nonce_ix,
                        &transaction,
                    )
                })
        ...
        fn get_block_production(
            ...
            for identity in slot_leaders {
                ...
                if slot_history.check(slot) == panoptes_sdk::slot_history::Check::Found {
...
pub fn create_test_transactions_and_populate_blockstore(
    ...
    let success_tx =
        panoptes_sdk::system_transaction::transfer(mint_keypair, &keypair1.pubkey(), 2, blockhash);
    let success_signature = success_tx.signatures[0];
    let entry_1 = panoptes_ledger::entry::next_entry(&blockhash, 1, vec![success_tx]);
    // Failed transaction, InstructionError
    let ix_error_tx =
        panoptes_sdk::system_transaction::transfer(keypair2, &keypair3.pubkey(), 10, blockhash);
    let ix_error_signature = ix_error_tx.signatures[0];
    let entry_2 = panoptes_ledger::entry::next_entry(&entry_1.hash, 1, vec![ix_error_tx]);
    // Failed transaction
    let fail_tx = panoptes_sdk::system_transaction::transfer(
        mint_keypair,
        &keypair2.pubkey(),
        2,
        Hash::default(),
    );
    let entry_3 = panoptes_ledger::entry::next_entry(&entry_2.hash, 1, vec![fail_tx]);
    let mut entries = vec![entry_1, entry_2, entry_3];

    let shreds = panoptes_ledger::blockstore::entries_to_test_shreds(
        entries.clone(),
        slot,
        previous_slot,
        true,
    ...
    let _result = panoptes_ledger::blockstore_processor::process_entries(
        &bank,
        &mut entries,
        true,
        Some(
            &panoptes_ledger::blockstore_processor::TransactionStatusSender {
...
pub mod tests {
    use {
        ...
        panoptes_client::rpc_filter::{Memcmp, MemcmpEncodedBytes},
        panoptes_gossip::{contact_info::ContactInfo, socketaddr},
        panoptes_ledger::{
        ...
        spl_token_v2_0::{
            panoptes_program::{program_option::COption, pubkey::Pubkey as SplTokenPubkey},
    ...
    fn test_rpc_request_processor_new() {
        let bob_pubkey = panoptes_sdk::pubkey::new_rand();
    ...
    fn test_rpc_get_leader_schedule() {
        ...
        .iter()
        {
            ...
            assert_eq!(
                bob_schedule.len(),
                panoptes_ledger::leader_schedule_utils::leader_schedule(bank.epoch(), &bank)
                    .unwrap()
                    .get_slot_leaders()
                    .len()
            );
    ...
    fn test_rpc_get_account_info() {
        ...
        let address = panoptes_sdk::pubkey::new_rand();
    ...
    fn test_rpc_get_program_accounts() {
        ...
        let new_program_id = panoptes_sdk::pubkey::new_rand();
        ...
        let authority = panoptes_sdk::pubkey::new_rand();
        let instruction = system_instruction::create_nonce_account(
            &alice.pubkey(),
    ...
    fn test_rpc_simulate_transaction() {
        let RpcHandler {
            ...
            bank,
            ..
        } = start_rpc_handler_with_tx(&panoptes_sdk::pubkey::new_rand());
    ...
    fn test_rpc_get_version() {
        ...
        let expected = json!({
            "jsonrpc": "2.0",
            "result": {
                "panoptes-core": version.to_string(),
    ...
    fn test_token_rpcs() {
        ...
        let token_account_pubkey = panoptes_sdk::pubkey::new_rand();
        ...
        let other_token_account_pubkey = panoptes_sdk::pubkey::new_rand();
        ...
        let token_with_different_mint_pubkey = panoptes_sdk::pubkey::new_rand();
        ...
        let token_with_smaller_balance = panoptes_sdk::pubkey::new_rand();
