use {
    crate::client_error,
    panoptes_account_decoder::{parse_token::UiTokenAmount, UiAccount},
    panoptes_sdk::{
...
pub struct RpcVersionInfo {
    /// The current version of panoptes-core
    pub panoptes_core: String,
    /// first 4 bytes of the FeatureSet identifier
    pub feature_set: Option<u32>,
}
...
impl fmt::Debug for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.panoptes_core)
    }
}

impl fmt::Display for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if let Some(version) = self.panoptes_core.split_whitespace().next() {
