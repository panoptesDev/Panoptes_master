            ...
            parse_signer_source(&self.path)
                ...
                .map_err(|_| {
                    std::io::Error::new(
                        ...
                        format!(
                        "No default signer found, run \"panoptes-keygen new -o {}\" to create a new one",
                        self.path
                    ),
                    )
                })?;
        ...
        SignerSourceKind::Filepath(path) => match read_keypair_file(&path) {
            Err(e) => Err(std::io::Error::new(
                ...
                format!(
                    ...
                    Run \"panoptes-keygen new\" to create a keypair file: {}",
                    path, e
                ),
            )
            .into()),
...
mod tests {
    use super::*;
    use panoptes_remote_wallet::locator::Manufacturer;
