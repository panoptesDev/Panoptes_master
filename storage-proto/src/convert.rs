...
use panoptes_account_decoder::parse_token::{real_number_string_trimmed, UiTokenAmount};
use panoptes_sdk::{
    hash::Hash,
...
pub mod generated {
    include!(concat!(
        ...
        concat!("/proto/panoptes.storage.confirmed_block.rs")
    ));
}
...
pub mod tx_by_addr {
    include!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        concat!("/proto/panoptes.storage.transaction_by_addr.rs")
    ));
}
