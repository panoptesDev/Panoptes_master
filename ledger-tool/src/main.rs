...
use panoptes_ledger::entry::Entry;
use panoptes_ledger::{
...
use panoptes_stake_program::stake_state::{self, PointValue};
    ...
    match method {
        LedgerOutputMethod::Print => {
            ...
            for (transactions_index, transaction) in entry.transactions.iter().enumerate() {
                ...
                panoptes_cli_output::display::println_transaction(
                    transaction,
                    &transaction_status,
                    "      ",
                    None,
...
fn analyze_column<
    T: panoptes_ledger::blockstore_db::Column + panoptes_ledger::blockstore_db::ColumnName,
>(
    db: &Database,
    name: &str,
    key_size: usize,
        ...
        ("modify-genesis", Some(arg_matches)) => {
            ...
            if let Some(hashes_per_tick) = arg_matches.value_of("hashes_per_tick") {
                genesis_config.poh_config.hashes_per_tick = match hashes_per_tick {
                    // Note: Unlike `panoptes-genesis`, "auto" is not supported here.
                    "sleep" => None,
            ...
            create_new_ledger(
                &output_directory,
                &genesis_config,
                panoptes_runtime::hardened_unpack::MAX_GENESIS_ARCHIVE_UNPACKED_SIZE,
            ...
            match load_bank_forks(
                ...
                Ok((bank_forks, _leader_schedule_cache, _snapshot_hash)) => {
                    ...
                    if let Some(bootstrap_validator_pubkeys) = bootstrap_validator_pubkeys {
                        ...
                        for (address, mut account) in bank
                            .get_program_accounts(&panoptes_vote_program::id())
                            .unwrap()
                            .into_iter()
                        {
                            account.set_lamports(0);
                ...
                Ok((bank_forks, _leader_schedule_cache, _snapshot_hash)) => {
                    ...
                    let accounts: BTreeMap<_, _> = bank
                        ...
                        .filter(|(pubkey, _account, _slot)| {
                            include_sysvars || !panoptes_sdk::sysvar::is_sysvar_id(pubkey)
                        })
            ...
            match load_bank_forks(
                ...
                Ok((bank_forks, _leader_schedule_cache, _snapshot_hash)) => {
                    ...
                    if arg_matches.is_present("warp_epoch") {
                        ...
                        use panoptes_stake_program::stake_state::InflationPointCalculationEvent;
                        ...
                        for (pubkey, warped_account) in all_accounts {
                            ...
                            // inflation.
                            if panoptes_sdk::sysvar::is_sysvar_id(&pubkey) {
                                continue;
                            }
