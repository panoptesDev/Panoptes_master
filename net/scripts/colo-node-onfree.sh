...
if [[ -f "${PANOPTES_LOCK_FILE}" ]]; then
  ...
  . "${PANOPTES_LOCK_FILE}"
  if [[ "${PANOPTES_LOCK_USER}" = "${PANOPTES_USER}"  || -n "${FORCE_DELETE}" ]]; then
    ...
    git clean -qxdff
    rm -f /panoptes-scratch/* /panoptes-scratch/.[^.]*
...
EOAK
    ...
    echo "Invalid user: expected \"${PANOPTES_LOCK_USER}\" got \"${PANOPTES_USER}\"" 1>&2
  fi
  exec 9>&-
fi
${RC}
