...
Creates a testnet dev metrics database
  ...
  -e              Assume database already exists and PANOPTES_METRICS_CONFIG is
...
host="https://metrics.panoptes.com:8086"
...
if $useEnv; then
  [[ -n $PANOPTES_METRICS_CONFIG ]] ||
    usage "PANOPTES_METRICS_CONFIG is not defined in the environment"
else
  username=$1
  ...
  PANOPTES_METRICS_CONFIG="host=$host,db=$netBasename,u=scratch_writer,p=topsecret"
fi

echo "export PANOPTES_METRICS_CONFIG=\"$PANOPTES_METRICS_CONFIG\"" >> "$configFile"
