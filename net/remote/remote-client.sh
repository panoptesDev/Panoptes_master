...
panoptes-bench-tps)
  net/scripts/rsync-retry.sh -vPrc \
    "$entrypointIp":~/panoptes/config/bench-tps"$clientIndex".yml ./client-accounts.yml
  clientCommand="\
    panoptes-bench-tps \
  ...
  "
  ;;
panoptes-bench-exchange)
  panoptes-keygen new --no-passphrase -fso bench.keypair
  net/scripts/rsync-retry.sh -vPrc \
    "$entrypointIp":~/panoptes/config/bench-exchange"$clientIndex".yml ./client-accounts.yml
  clientCommand="\
    panoptes-bench-exchange \
      --entrypoint $entrypointIp:8001 \
      --faucet $entrypointIp:9900 \
...
idle)
  ...
  net/scripts/rsync-retry.sh -vPrc \
    "$entrypointIp":~/panoptes/config/faucet.json ~/panoptes/
  exit 0
  ;;
*)
...
echo "@reboot ~/panoptes/on-reboot" | crontab -
