...
local|tar|skip)
  ...
  panoptes_cli=panoptes
  panoptes_gossip=panoptes-gossip
  panoptes_install=panoptes-install
  ;;
*)
...
(
  set -x
  $panoptes_cli --url http://"$sanityTargetIp":8899 validators
)
...
(
  ...
  $panoptes_gossip spy --entrypoint "$sanityTargetIp:8001" \
...
if $installCheck && [[ -r update_manifest_keypair.json ]]; then
  echo "--- $sanityTargetIp: panoptes-install test"

  (
    set -x
    rm -rf install-data-dir
    $panoptes_install init \
      --no-modify-path \
      --data-dir install-data-dir \
      --url http://"$sanityTargetIp":8899 \

    $panoptes_install info
  )
fi
