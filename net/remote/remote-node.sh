...
cat >> ~/panoptes/on-reboot <<EOF
  ...
  sudo RUST_LOG=info ~panoptes/.cargo/bin/panoptes-sys-tuner --user $(whoami) > sys-tuner.log 2>&1 &
  echo \$! > sys-tuner.pid

  (
    sudo PANOPTES_METRICS_CONFIG="$PANOPTES_METRICS_CONFIG" scripts/oom-monitor.sh
  ) > oom-monitor.log 2>&1 &
  ...
  if ${GPU_CUDA_OK} && [[ -e /dev/nvidia0 ]]; then
    echo Selecting panoptes-validator-cuda
    export PANOPTES_CUDA=1
  elif ${GPU_FAIL_IF_NONE} ; then
    echo "Expected GPU, found none!"
    export PANOPTES_GPU_MISSING=1
  fi
EOF
  ...
  bootstrap-validator)
    ...
    if [[ $skipSetup != true ]]; then
      clear_config_dir "$PANOPTES_CONFIG_DIR"
      ...
      setupValidatorKeypair() {
        ...
        if [[ -f net/keypairs/"$name".json ]]; then
          ...
          panoptes-keygen new --no-passphrase -so config/"$name".json
          if [[ "$name" =~ ^validator-identity- ]]; then
            name="${name//-identity-/-vote-}"
            panoptes-keygen new --no-passphrase -so config/"$name".json
            name="${name//-vote-/-stake-}"
        ...
        if [[ -n $internalNodesLamports ]]; then
          declare pubkey
          pubkey="$(panoptes-keygen pubkey config/"$name".json)"
...
EOF
      ...
      for i in $(seq 0 $((numBenchTpsClients-1))); do
        ...
        panoptes-bench-tps --write-client-keys config/bench-tps"$i".yml \
      ...
      for i in $(seq 0 $((numBenchExchangeClients-1))); do
        ...
        panoptes-bench-exchange --batch-size 1000 --fund-amount 20000 \
      ...
      if [[ "$extraPrimordialStakes" -gt 0 ]]; then
        ...
        for i in $(seq "$extraPrimordialStakes"); do
          args+=(--bootstrap-validator "$(panoptes-keygen pubkey "config/validator-identity-$i.json")"
                                       "$(panoptes-keygen pubkey "config/validator-vote-$i.json")"
                                       "$(panoptes-keygen pubkey "config/validator-stake-$i.json")"
          )
        done
      fi
      ...
      if [[ -n "$maybeWarpSlot" ]]; then
        ...
        panoptes-ledger-tool -l config/bootstrap-validator create-snapshot 0 config/bootstrap-validator $maybeWarpSlot
      fi

      panoptes-ledger-tool -l config/bootstrap-validator shred-version --max-genesis-archive-unpacked-size 1073741824 | tee config/shred-version

      if [[ -n "$maybeWaitForSupermajority" ]]; then
        bankHash=$(panoptes-ledger-tool -l config/bootstrap-validator bank-hash)
    ...
    if [[ "$tmpfsAccounts" = "true" ]]; then
      args+=(--accounts /mnt/panoptes-accounts)
    fi
...
EOF
  ...
  validator|blockstreamer)
    ...
    if [[ $skipSetup != true ]]; then
      ...
      if [[ $nodeType = blockstreamer ]]; then
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/panoptes/config/blockstreamer-identity.json "$PANOPTES_CONFIG_DIR"/validator-identity.json
      else
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/panoptes/config/validator-identity-"$nodeIndex".json "$PANOPTES_CONFIG_DIR"/validator-identity.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/panoptes/config/validator-stake-"$nodeIndex".json "$PANOPTES_CONFIG_DIR"/stake-account.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/panoptes/config/validator-vote-"$nodeIndex".json "$PANOPTES_CONFIG_DIR"/vote-account.json
      fi
      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/panoptes/config/shred-version "$PANOPTES_CONFIG_DIR"/shred-version

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/panoptes/config/bank-hash "$PANOPTES_CONFIG_DIR"/bank-hash || true

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/panoptes/config/faucet.json "$PANOPTES_CONFIG_DIR"/faucet.json
    fi
    ...
    args=(
      ...
      --gossip-port 8001
      --rpc-port 8899
      --expected-shred-version "$(cat "$PANOPTES_CONFIG_DIR"/shred-version)"
    )
    if [[ $nodeType = blockstreamer ]]; then
      args+=(
        --blockstream /tmp/panoptes-blockstream.sock
        --no-voting
        --dev-no-sigverify
    ...
    if [[ ! -f "$PANOPTES_CONFIG_DIR"/validator-identity.json ]]; then
      panoptes-keygen new --no-passphrase -so "$PANOPTES_CONFIG_DIR"/validator-identity.json
    fi
    args+=(--identity "$PANOPTES_CONFIG_DIR"/validator-identity.json)
    if [[ ! -f "$PANOPTES_CONFIG_DIR"/vote-account.json ]]; then
      panoptes-keygen new --no-passphrase -so "$PANOPTES_CONFIG_DIR"/vote-account.json
    fi
    args+=(--vote-account "$PANOPTES_CONFIG_DIR"/vote-account.json)
    ...
    if [[ -r "$PANOPTES_CONFIG_DIR"/bank-hash ]]; then
      args+=(--expected-bank-hash "$(cat "$PANOPTES_CONFIG_DIR"/bank-hash)")
    fi
    ...
    scp "$entrypointIp":~/panoptes/config/faucet.json "$PANOPTES_CONFIG_DIR"/
    if [[ $nodeType = blockstreamer ]]; then
...
EOF
    ...
    if [[ $skipSetup != true && $nodeType != blockstreamer && -z $maybeSkipAccountsCreation ]]; then
      ...
      panoptes --url http://"$entrypointIp":8899 catchup config/validator-identity.json
      ...
      if [[ ${extraPrimordialStakes} -eq 0 ]]; then
        ...
        multinode-demo/delegate-stake.sh --vote-account "$PANOPTES_CONFIG_DIR"/vote-account.json \
                                         --stake-account "$PANOPTES_CONFIG_DIR"/stake-account.json \
