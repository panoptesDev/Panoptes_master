...
PANOPTES_ROOT="$netDir"/..
# shellcheck source=scripts/configure-metrics.sh
source "$PANOPTES_ROOT"/scripts/configure-metrics.sh
...
PANOPTES_CONFIG_DIR=$PANOPTES_ROOT/config
...
_setup_secondary_mount() {
  ...
  (
    ...
    if [[ -d $SECONDARY_DISK_MOUNT_POINT ]] && \
      ...
      rm -rf "$PANOPTES_CONFIG_DIR"
      ln -sfT $SECONDARY_DISK_MOUNT_POINT/config "$PANOPTES_CONFIG_DIR"
    fi
  )
}
