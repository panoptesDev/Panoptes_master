...
#[macro_use]
extern crate panoptes_exchange_program;
...
use panoptes_genesis::{genesis_accounts::add_genesis_accounts, Base64Account};
use panoptes_ledger::{
    blockstore::create_new_ledger, blockstore_db::AccessType, poh::compute_hashes_per_tick,
};
use panoptes_runtime::hardened_unpack::MAX_GENESIS_ARCHIVE_UNPACKED_SIZE;
use panoptes_sdk::{
...
use panoptes_vote_program::vote_state::{self, VoteState};
use std::{
    collections::HashMap,
    error,
    fs::File,
...
fn main() -> Result<(), Box<dyn error::Error>> {
    let default_faucet_pubkey = panoptes_cli_config::Config::default().keypair_path;
    ...
    panoptes_stake_program::add_genesis_accounts(&mut genesis_config);
    if genesis_config.cluster_type == ClusterType::Development {
        panoptes_runtime::genesis_utils::activate_all_features(&mut genesis_config);
    }
...
mod tests {
    ...
    fn test_append_primordial_accounts_to_genesis() {
        ...
        genesis_accounts.insert(
            panoptes_sdk::pubkey::new_rand().to_string(),
            Base64Account {
                owner: panoptes_sdk::pubkey::new_rand().to_string(),
                balance: 2,
                executable: false,
