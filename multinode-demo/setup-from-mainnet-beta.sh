...
rm -rf "$PANOPTES_CONFIG_DIR"/latest-mainnet-beta-snapshot
mkdir -p "$PANOPTES_CONFIG_DIR"/latest-mainnet-beta-snapshot
(
  cd "$PANOPTES_CONFIG_DIR"/latest-mainnet-beta-snapshot || exit 1
  set -x
  wget http://api.mainnet-beta.panoptes.com/genesis.tar.bz2
  wget --trust-server-names http://api.mainnet-beta.panoptes.com/snapshot.tar.bz2
)

snapshot=$(ls "$PANOPTES_CONFIG_DIR"/latest-mainnet-beta-snapshot/snapshot-[0-9]*-*.tar.zst)
if [[ -z $snapshot ]]; then
...
$panoptes_ledger_tool create-snapshot \
  --ledger "$PANOPTES_CONFIG_DIR"/latest-mainnet-beta-snapshot \
