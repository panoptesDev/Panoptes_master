...
if [[ "$PANOPTES_GPU_MISSING" -eq 1 ]]; then
...
if [[ -n $REQUIRE_LEDGER_DIR ]]; then
  ...
  PANOPTES_CONFIG_DIR="$ledger_dir"
fi
...
if [[ -z "$ledger_dir" ]]; then
  ledger_dir="$PANOPTES_CONFIG_DIR/validator$label"
fi
mkdir -p "$ledger_dir"
...
if [[ -n $PANOPTES_CUDA ]]; then
  program=$panoptes_validator_cuda
else
  program=$panoptes_validator
fi
...
wallet() {
  (
    set -x
    $panoptes_cli --keypair "$identity" --url "$rpc_url" "$@"
  )
}
  ...
  if ! wallet vote-account "$vote_account"; then
    if ((airdrops_enabled)); then
      ...
      (
        set -x
        $panoptes_cli \
          --keypair "$PANOPTES_CONFIG_DIR/faucet.json" --url "$rpc_url" \
...
rpc_url=$($panoptes_gossip rpc-url --timeout 180 --entrypoint "$gossip_entrypoint")

[[ -r "$identity" ]] || $panoptes_keygen new --no-passphrase -so "$identity"
[[ -r "$vote_account" ]] || $panoptes_keygen new --no-passphrase -so "$vote_account"
