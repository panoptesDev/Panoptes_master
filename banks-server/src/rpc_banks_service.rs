//! The `rpc_banks_service` module implements the Panoptes Banks RPC API.

use crate::banks_server::start_tcp_server;
use futures::{future::FutureExt, pin_mut, prelude::stream::StreamExt, select};
use panoptes_runtime::{bank_forks::BankForks, commitment::BlockCommitmentCache};
use std::{
    net::SocketAddr,
    sync::{
...
impl RpcBanksService {
    ...
    pub fn new(
        ...
        let thread_hdl = Builder::new()
            .name("panoptes-rpc-banks".to_string())
            .spawn(move || {
                Self::run(
                    listen_addr,
                    tpu_addr,
