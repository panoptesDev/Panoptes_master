...
use lru::LruCache;
use panoptes_client::rpc_response::SlotUpdate;
use panoptes_gossip::{
    cluster_info::{compute_retransmit_peers, ClusterInfo, DATA_PLANE_FANOUT},
    contact_info::ContactInfo,
};
use panoptes_ledger::shred::{get_shred_slot_index_type, ShredFetchStats};
use panoptes_ledger::{
    blockstore::{Blockstore, CompletedSlotsReceiver},
    leader_schedule_cache::LeaderScheduleCache,
};
use panoptes_measure::measure::Measure;
use panoptes_metrics::inc_new_counter_error;
use panoptes_perf::packet::{Packet, Packets};
use panoptes_rpc::{
    max_slots::MaxSlots, rpc_completed_slots_service::RpcCompletedSlotsService,
    rpc_subscriptions::RpcSubscriptions,
};
use panoptes_runtime::{bank::Bank, bank_forks::BankForks};
use panoptes_sdk::{clock::Slot, epoch_schedule::EpochSchedule, pubkey::Pubkey, timing::timestamp};
use panoptes_streamer::streamer::PacketReceiver;
use std::{
    cmp,
...
pub fn retransmitter(
    ...
    (0..sockets.len())
        .map(|s| {
            ...
            Builder::new()
                .name("panoptes-retransmitter".to_string())
                .spawn(move || {
...
mod tests {
    ...
    use panoptes_ledger::blockstore_processor::{process_blockstore, ProcessOptions};
    use panoptes_ledger::create_new_tmp_ledger;
    use panoptes_ledger::genesis_utils::{create_genesis_config, GenesisConfigInfo};
    use panoptes_ledger::shred::Shred;
    use panoptes_net_utils::find_available_port_in_range;
    use panoptes_perf::packet::{Packet, Packets};
    ...
    fn test_skip_repair() {
        ...
        let mut me = ContactInfo::new_localhost(&panoptes_sdk::pubkey::new_rand(), 0);
        ...
        // the test.
        let other = std::iter::repeat_with(panoptes_sdk::pubkey::new_rand)
            .find(|pk| me.id < *pk)
            .unwrap();
        ...
        panoptes_streamer::packet::recv_from(&mut packets, &me_retransmit, 1).unwrap();
