...
use panoptes_metrics::inc_new_counter_debug;
use panoptes_perf::packet::{limited_deserialize, Packets, PacketsRecycler};
use panoptes_sdk::{
    clock::Slot,
    pubkey::Pubkey,
...
use panoptes_streamer::streamer::{PacketReceiver, PacketSender};
use std::{
...
impl ServeRepair {
    ...
    pub fn listen(
        ...
        Builder::new()
            .name("panoptes-repair-listen".to_string())
            .spawn(move || {
    ...
    pub fn repair_request(
        ...
        let nonce =
            outstanding_requests.add_request(repair_request, panoptes_sdk::timing::timestamp());
    ...
    pub fn repair_request_duplicate_compute_best_peer(
        ...
        let n = weighted_best(&weights, panoptes_sdk::pubkey::new_rand().to_bytes());
...
mod tests {
    ...
    use panoptes_gossip::{socketaddr, socketaddr_any};
    ...
    use panoptes_perf::packet::Packet;
    use panoptes_sdk::{hash::Hash, pubkey::Pubkey, timing::timestamp};
    ...
    fn run_window_request(slot: Slot, nonce: Nonce) {
        ...
        let ledger_path = get_tmp_ledger_path!();
        {
            let blockstore = Arc::new(Blockstore::open(&ledger_path).unwrap());
            let me = ContactInfo {
                id: panoptes_sdk::pubkey::new_rand(),
    ...
    fn window_index_request() {
        ...
        let me = ContactInfo::new_localhost(&panoptes_sdk::pubkey::new_rand(), timestamp());
    ...
    fn test_repair_with_repair_validators() {
        ...
        for pubkey in &[panoptes_sdk::pubkey::new_rand(), me.id] {
