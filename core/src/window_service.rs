...
use panoptes_metrics::{inc_new_counter_debug, inc_new_counter_error};
use panoptes_perf::packet::Packets;
use panoptes_rayon_threadlimit::get_thread_count;
use panoptes_runtime::{bank::Bank, bank_forks::BankForks};
use panoptes_sdk::{clock::Slot, packet::PACKET_DATA_SIZE, pubkey::Pubkey, timing::duration_as_ms};
use panoptes_streamer::streamer::PacketSender;
...
impl WindowService {
    ...
    fn start_check_duplicate_thread(
        ...
        let handle_error = || {
            inc_new_counter_error!("panoptes-check-duplicate-error", 1, 1);
        };
        Builder::new()
            .name("panoptes-check-duplicate".to_string())
            .spawn(move || loop {
    ...
    fn start_window_insert_thread(
        ...
        let handle_error = || {
            inc_new_counter_error!("panoptes-window-insert-error", 1, 1);
        };

        Builder::new()
            .name("panoptes-window-insert".to_string())
            .spawn(move || {
            ...
            + std::marker::Sync,
    {
        ...
        let blockstore = blockstore.clone();
        Builder::new()
            .name("panoptes-window".to_string())
            .spawn(move || {
                ...
                let handle_error = || {
                    inc_new_counter_error!("panoptes-window-error", 1, 1);
                };
...
mod test {
    ...
    fn test_should_retransmit_and_persist() {
        let me_id = panoptes_sdk::pubkey::new_rand();
