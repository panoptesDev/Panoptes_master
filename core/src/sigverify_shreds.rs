...
use panoptes_ledger::leader_schedule_cache::LeaderScheduleCache;
use panoptes_ledger::shred::Shred;
use panoptes_ledger::sigverify_shreds::verify_shreds_gpu;
use panoptes_perf::{self, packet::Packets, recycler_cache::RecyclerCache};
...
impl SigVerifier for ShredSigVerifier {
    fn verify_batch(&self, mut batches: Vec<Packets>) -> Vec<Packets> {
        ...
        panoptes_perf::sigverify::mark_disabled(&mut batches, &r);
        batches
    }
}
...
pub mod tests {
    use super::*;
    use panoptes_ledger::genesis_utils::create_genesis_config_with_leader;
    use panoptes_ledger::shred::{Shred, Shredder};
