use {
    ...
    panoptes_gossip::{cluster_info::Node, gossip_service::discover_cluster, socketaddr},
    panoptes_ledger::{blockstore::create_new_ledger, create_new_tmp_ledger},
    panoptes_net_utils::PortRange,
...
impl TestValidatorGenesis {
    ...
    pub fn add_account_with_file_data(
        ...
        self.add_account(
            ...
            AccountSharedData::from(Account {
                lamports,
                data: panoptes_program_test::read_file(
                    panoptes_program_test::find_file(filename).unwrap_or_else(|| {
    ...
    pub fn add_program(&mut self, program_name: &str, program_id: Pubkey) -> &mut Self {
        let program_path = panoptes_program_test::find_file(&format!("{}.so", program_name))
...
impl TestValidator {
    ...
    fn initialize_ledger(
        ...
        for (address, account) in panoptes_program_test::programs::spl_programs(&config.rent) {
            accounts.entry(address).or_insert(account);
        }
        for program in &config.programs {
            let data = panoptes_program_test::read_file(&program.program_path);
            accounts.insert(
                program.program_id,
                AccountSharedData::from(Account {
        ...
        let mut genesis_config = create_genesis_config_with_leader_ex(
            ...
            config.rent,
            panoptes_sdk::genesis_config::ClusterType::Development,
        ...
        let ledger_path = match &config.ledger_path {
            ...
            Some(ledger_path) => {
                ...
                let _ = create_new_ledger(
                    ...
                    panoptes_ledger::blockstore_db::AccessType::PrimaryOnly,
                )
                .map_err(|err| {
                    format!(
    ...
    fn start(
        ...
        // Needed to avoid panics in `panoptes-responder-gossip` in tests that create a number of
