...
mod tests {
    ...
    use panoptes_core::snapshot_packager_service::{PendingSnapshotPackage, SnapshotPackagerService};
    use panoptes_gossip::{cluster_info::ClusterInfo, contact_info::ContactInfo};
            ...
            .spawn(move || {
                while let Ok(mut snapshot_package) = receiver.recv() {
                    ...
                    let snapshot_package =
                        panoptes_runtime::snapshot_utils::process_accounts_package_pre(
                            snapshot_package,
                            Some(&thread_pool),
                        );
