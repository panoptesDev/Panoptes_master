//! The `faucet` module provides an object for launching a Panoptes Faucet,
//! which is the custodian of any remaining lamports in a mint.
//! The Panoptes Faucet builds and sends airdrop transactions,
...
use {
    ...
    panoptes_metrics::datapoint_info,
    panoptes_sdk::{
        hash::Hash,
...
impl Faucet {
    ...
    pub fn new_with_allowed_ips(
        ...
        if let Some((per_request_cap, per_time_cap)) = per_request_cap.zip(per_time_cap) {
            if per_time_cap < per_request_cap {
                warn!(
                    "per_time_cap {} PANO < per_request_cap {} PANO; \
    ...
    /// transaction with the memo: "request too large; req: <REQUEST> PANO cap: <CAP> PANO"
...
mod tests {
    use super::*;
    use panoptes_sdk::system_instruction::SystemInstruction;
