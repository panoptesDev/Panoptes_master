use {
    ...
    log::*,
    panoptes_clap_utils::input_parsers::{lamports_of_sol, value_of},
    panoptes_faucet::{
        faucet::{run_faucet, Faucet, FAUCET_PORT},
        socketaddr,
    },
    panoptes_sdk::signature::read_keypair_file,
    std::{
        collections::HashSet,
...
#[tokio::main]
async fn main() {
    let default_keypair = panoptes_cli_config::Config::default().keypair_path;

    panoptes_logger::setup_with_default("panoptes=info");
    panoptes_metrics::set_panic_hook("faucet");
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("per_time_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for time slice, in PANO"),
        )
        .arg(
            Arg::with_name("per_request_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for a single request, in PANO"),
        )
        .arg(
