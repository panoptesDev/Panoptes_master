...
Panoptes维护着几个不同用途的集群。

在开始之前，请确保已首先安装了[Panoptes命令行工具](cli/install-panoptes-cli-tools.md)
...
- Devnet可以作为希望将Panoptes进行测试的任何人，用户，代币持有者，应用开发者或验证者的游乐场。
- 应用程序开发人员应针对Devnet。
- 潜在的验证者应首先针对Devnet。
...
`--trusted-validator`由 Panoptes 运行
...
- 测试网 Gossip 入口： `entrypoint.testnet.panoptes.com:8001`
- Testnet的指标环境变量：
```bash
...
```
- Testnet 的 RPC URL: `https://api.testnet.panoptes.com`
...
- `5D1fNXzv5NjV1ysLjirC4WY92RNsVH18vjmcszZd8on` - testnet.panoptes.com (Panoptes)
- `ta1Uvfb7W5BRPrdGnhP9RmeCKzBySGM1hTE4rBRy6T` - Break RPC 节点 (Panoptes)
...
- Mainnet Beta 的 Gossip 入口： `entrypoint.mainnet-beta.panoptes.com:8001`
- Mainnet Beta的指标环境变量：
```bash
export PANOPTES_METRICS_CONFIG="host=https://metrics.panoptes.com:8086,db=mainnet-beta,u=mainnet-beta_write,p=password"
```
- Mainnet Beta 的 RPC URL： `https://api.mainnet-beta.panoptes.com`
...
所有的四个 `--trusted-validator（可信验证节点）` 由 Panoptes 运行
