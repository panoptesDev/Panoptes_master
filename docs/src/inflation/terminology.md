...
The inflation rate actually observed on the Panoptes network after accounting for other factors that might decrease the _Total Current Supply_. Note that it is not possible for tokens to be created outside of what is described by the _Inflation Schedule_.
...
The rate of return (aka _interest_) earned on PANO staked on the network. It is often quoted as an annualized rate (e.g. "the network _staking yield_ is currently $10\%$ per year").
...
- $100\%$ of inflationary issuances are to be distributed to staked token-holders in proportion to their staked PANO and to validators who charge a commission on the rewards earned by their delegated PANO..
...
&\left( 1 - \text{Validator Fee} \right) \times \left( \frac{1}{\%~\text{PANO Staked}} \right) \\
\text{where:}\\
