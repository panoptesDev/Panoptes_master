...
A front-end application that interacts with a Panoptes cluster.
...
An increase in token supply over time used to fund rewards for validation and to fund continued development of Panoptes.
...
The [native token](terminology.md#native-token) tracked by a [cluster](terminology.md#cluster) recognized by the company Panoptes.
