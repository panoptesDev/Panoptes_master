---
title: Panoptes Test Validator
---
During early stage development, it is often convenient to target a cluster with
fewer restrictions and more configuration options than the public offerings
provide. This is easily achieved with the `panoptes-test-validator` binary, which
...
## Installation
The `panoptes-test-validator` binary ships with the Panoptes CLI Tool Suite.
[Install](/cli/install-panoptes-cli-tools) before continuing.

## Running
First take a look at the configuration options
```
panoptes-test-validator --help
```

Next start the test validator
```
panoptes-test-validator
```
...
Leave `panoptes-test-validator` running in its own terminal. When it is no longer
needed, it can be stopped with ctrl-c.

## Interacting
Open a new terminal to interact with a [running](#running) `panoptes-test-validator`
instance using other binaries from the Panoptes CLI Tool Suite or your own client
software.

#### Configure the CLI Tool Suite to target a local cluster by default
```
panoptes config set --url http://127.0.0.1:8899
```

#### Verify the CLI Tool Suite configuration
```
panoptes genesis-hash
```
* **NOTE:** The result should match the `Genesis Hash:` field in the
`panoptes-test-validator` status output
...
wallet does not yet exist. Create it with `panoptes-keygen new`.
* **NOTE:** If the wallet has a zero PANO balance, airdrop some localnet PANO with
`panoptes airdrop 10`

#### Perform a basic transfer transaction
```
panoptes transfer EPhgPANa5Rh2wa4V2jxt7YbtWa3Uyw4sTeZ13cQjDDB8 1
```
