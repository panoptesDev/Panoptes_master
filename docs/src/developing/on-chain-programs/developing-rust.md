...
Panoptes supports writing on-chain programs using the
[Rust](https://www.rust-lang.org/) programming language.

## Project Layout

Panoptes Rust programs follow the typical [Rust project
...
Panoptes Rust programs may depend directly on each other in order to gain access
...
- [Define the
  feature](https://github.com/panoptes-labs/panoptes-program-library/blob/a5babd6cbea0d3f29d8c57d2ecbbd2a2bd59c8a9/token/program/Cargo.toml#L12)
- [Exclude the
  entrypoint](https://github.com/panoptes-labs/panoptes-program-library/blob/a5babd6cbea0d3f29d8c57d2ecbbd2a2bd59c8a9/token/program/src/lib.rs#L12)

Then when other programs include this program as a dependency, they should do so
using the `exclude_entrypoint` feature.

- [Include without
  entrypoint](https://github.com/panoptes-labs/panoptes-program-library/blob/a5babd6cbea0d3f29d8c57d2ecbbd2a2bd59c8a9/token-swap/program/Cargo.toml#L19)

## Project Dependencies

At a minimum, Panoptes Rust programs must pull in the
[panoptes-program](https://crates.io/crates/panoptes-program) crate.

Panoptes BPF programs have some [restrictions](#restrictions) that may prevent the
...
- Crates may depend on `rand` which is not supported in Panoptes's deterministic
...
- Install the latest Panoptes command-line tools from
  https://docs.panoptes.com/cli/install-panoptes-cli-tools
...
To build a specific program, such as SPL Token, for the Panoptes BPF target which
...
Panoptes programs can be unit tested via the traditional `cargo test` mechanism by
...
[`program-test`](https://crates.io/crates/panoptes-program-test) crate. The
...
example](https://github.com/panoptes-labs/panoptes-program-library/blob/master/examples/rust/sysvar/tests/functional.rs)
shows how an instruction containing syavar account is sent and processed by the
program.

## Program Entrypoint

Programs export a known entrypoint symbol which the Panoptes runtime looks up and
calls when invoking a program. Panoptes supports multiple [versions of the BPF
...
- [BPF Loader's entrypoint
  macro](https://github.com/panoptes-labs/panoptes/blob/7ddf10e602d2ed87a9e3737aa8c32f1db9f909d8/sdk/program/src/entrypoint.rs#L46)
- [BPF Loader deprecated's entrypoint
  macro](https://github.com/panoptes-labs/panoptes/blob/7ddf10e602d2ed87a9e3737aa8c32f1db9f909d8/sdk/program/src/entrypoint_deprecated.rs#L37)
...
entrypoint](https://github.com/panoptes-labs/example-helloworld/blob/c1a7247d87cd045f574ed49aec5d160aefc45cf2/src/program-rust/src/lib.rs#L15)
...
- [BPF Loader
  deserialization](https://github.com/panoptes-labs/panoptes/blob/7ddf10e602d2ed87a9e3737aa8c32f1db9f909d8/sdk/program/src/entrypoint.rs#L104)
- [BPF Loader deprecated
  deserialization](https://github.com/panoptes-labs/panoptes/blob/7ddf10e602d2ed87a9e3737aa8c32f1db9f909d8/sdk/program/src/entrypoint_deprecated.rs#L56)
...
[AccountInfo](https://github.com/panoptes-labs/panoptes/blob/7ddf10e602d2ed87a9e3737aa8c32f1db9f909d8/sdk/program/src/account_info.rs#L10)
...
[`global_allocator`](https://github.com/panoptes-labs/panoptes/blob/8330123861a719cd7a79af0544617896e7f00ce3/sdk/program/src/entrypoint.rs#L50)
...
- String formatting should be avoided since it is also computationally
  expensive.
- No support for `println!`, `print!`, the Panoptes [logging helpers](#logging)
...
`get-random` support for Panoptes. The error will typically look like this:
...
the helper macro
[`msg!`](https://github.com/panoptes-labs/panoptes/blob/6705b5a98c076ac08f3991bb8a6f9fcb280bf51e/sdk/program/src/log.rs#L33)
is provided.
...
Use the system call
[`sol_log_compute_units()`](https://github.com/panoptes-labs/panoptes/blob/d3a3a7548c857f26ec2cb10e270da72d373020ec/sdk/program/src/log.rs#L102)
...
The [Panoptes Program Library
github](https://github.com/panoptes-labs/panoptes-program-library/tree/master/examples/rust)
