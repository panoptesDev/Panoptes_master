...
The Panoptes runtime allows programs to call each other via a mechanism called
...
`invoke()` is built into Panoptes's runtime and is responsible for routing the
...
Panoptes, they must each transfer their wager's assets to some intermediary that
will honor their agreement. Currently, there is no way to implement this
intermediary as a program in Panoptes because the intermediary program cannot
