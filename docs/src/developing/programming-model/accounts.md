...
In the same way that a Linux user uses a path to look up a file, a Panoptes client
...
Accordingly, the Panoptes SDK [checks the sysvar account's validity during
deserialization](https://github.com/panoptes-labs/panoptes/blob/a95675a7ce1651f7b59443eb146b356bc4b3f374/sdk/program/src/sysvar/mod.rs#L65).
A alternative and safer way to read a sysvar is via the sysvar's [`get()`
function](https://github.com/panoptes-labs/panoptes/blob/64bfc14a75671e4ec3fe969ded01a599645080eb/sdk/program/src/sysvar/mod.rs#L73)
...
Keeping accounts alive on Panoptes incurs a storage cost called _rent_ because the
...
Rent can also be estimated via the [`panoptes rent` CLI subcommand](cli/usage.md#panoptes-rent)
