...
Panoptes nodes accept HTTP requests using the [JSON-RPC 2.0](https://www.jsonrpc.org/specification) specification.

To interact with a Panoptes node inside a JavaScript application, use the
[panoptes-web3.js](https://github.com/panoptes-labs/panoptes-web3.js) library, which
...
- Transaction: A list of Panoptes instructions signed by a client keypair to authorize those actions.
- Signature: An Ed25519 signature of transaction's payload data including instructions. This can be used to identify transactions.

## Configuring State Commitment

For preflight checks and transaction processing, Panoptes nodes choose which bank
...
1. If one or more `--trusted-validator` arguments are provided to `panoptes-validator`, "ok" is returned
...
**NEW: This method is only available in panoptes-core v1.7 or newer. Please use
[getConfirmedBlock](jsonrpc-api.md#getconfirmedblock) for panoptes-core v1.6**
...
- `<object>` - if block is confirmed, an object with the following fields:
  ...
  - `transactions: <array>` - present if "full" transaction details are requested; an array of JSON objects containing:
    ...
    - `meta: <object>` - transaction status metadata object, containing `null` or:
      - `err: <object | null>` - Error if transaction failed, null if transaction succeeded. [TransactionError definitions](https://github.com/panoptes-labs/panoptes/blob/master/sdk/src/transaction.rs#L24)
...
Transactions are quite different from those on other blockchains. Be sure to review [Anatomy of a Transaction](developing/programming-model/transactions.md) to learn about transactions on Panoptes.
...
The Panoptes runtime records the cross-program instructions that are invoked during transaction processing and makes these available for greater transparency of what was executed on-chain per transaction instruction. Invoked instructions are grouped by the originating transaction instruction and are listed in order of processing.
...
**NEW: This method is only available in panoptes-core v1.7 or newer. Please use
[getConfirmedBlocks](jsonrpc-api.md#getconfirmedblocks) for panoptes-core v1.6**
...
**NEW: This method is only available in panoptes-core v1.7 or newer. Please use
[getConfirmedBlocksWithLimit](jsonrpc-api.md#getconfirmedblockswithlimit) for panoptes-core v1.6**
...
`panoptes-validator`, "ok" is returned when the node has within
...
**NEW: This method is only available in panoptes-core v1.7 or newer. Please use
[getConfirmedSignaturesForAddress2](jsonrpc-api.md#getconfirmedsignaturesforaddress2) for panoptes-core v1.6**
...
- `<object>` - (optional) Configuration object containing the following field:
  - `searchTransactionHistory: <bool>` - if true, a Panoptes node will search its ledger cache for any signatures not found in the recent status cache
...
**NEW: This method is only available in panoptes-core v1.7 or newer. Please use
[getConfirmedTransaction](jsonrpc-api.md#getconfirmedtransaction) for panoptes-core v1.6**
...
Returns the current panoptes versions running on the node
...
- `panoptes-core`, software version of panoptes-core
...
Result:
```json
{"jsonrpc":"2.0","result":{"panoptes-core": "1.7.3"},"id":1}
```
...
- `err: <object | string | null>` - Error if transaction failed, null if transaction succeeded. [TransactionError definitions](https://github.com/panoptes-labs/panoptes/blob/master/sdk/src/transaction.rs#L24)
...
This method is expected to be removed in panoptes-core v1.8
