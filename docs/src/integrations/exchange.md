---
title: 添加 Panoptes 到您的交易所
---

本指南描述了如何将 Panoptes 的原生代币 PANO 添加到某个加密货币交易所。
...
这样设置可以让您：
- 为 Panoptes mainnet-beta 集群设置一个可信的网关来获取数据和提交取现交易
- 完全控制保留历史区块数据的多少
- 即使某个节点失败仍然保持您的服务可用性

Panoptes 节点需要较高的计算力来处理我们的快速区块和高 TPS 。  关于具体要求，请参阅[硬件建议](../running-validator/validator-reqs.md)。

运行一个 api 节点：

1. [安装 Panoptes 命令行工具](../cli/install-panoptes-cli-tools.md)
2. 启动验证节点时至少使用以下参数：
...
`--entrypoint` and `--experted-genesis-hash` 参数都针对您正在加入的集群。 [主网 Beta 的当前参数](../clusters.md#example-panoptes-validator-command-line-2)

`--limit-ledger-size` 参数允许您指定保留节点的多少个账本 [shreds](../terminology.md#shred) 在磁盘上。 如果您没有配置该参数，验证节点将保留整个账本直到磁盘空间满了为止。  保持账本磁盘使用量的默认值小于 500GB。  如果需要，可以通过添加参数到 `--limit-ledger-size` 来增加或减少磁盘的使用。 查看 `panoptes-validator --help` 来配置 `--limit-ledger-size` 所使用的默认限制值。  关于选择一个普通限制值的更多信息请参看 [这里](https://github.com/panoptes-labs/panoptes/blob/583cec922b6107e0f85c7e14cb5e642bc7dfb340/core/src/ledger_cleanup_service.rs#L15-L26).
...
我们建议将每个节点配置退出时自动重启，以确保尽可能少地丢失数据。 把 Panoptes 软件运行为一个系统服务是很好的选择。

对于监控，我们提供[`panoptes-watchtower`](https://github.com/panoptes-labs/panoptes/blob/master/watchtower/README.md)，它可以监视您的验证节点，并且通过 `panoptes-validator` 检测节点是否不健康。 它可以直接配置 Slack、Telegram 、Discord 或 Twillio 来提醒您。 详情请运行 `panoptes-watchtower --help`。

```bash
panoptes-watchtower --validator-identity <YOUR VALIDATOR IDENTITY>
```
...
默认情况下，您的每个节点都通过可信验证节点提供的快照启动。 这个快照反映了区块链当前的状态，但不包含完整的历史帐本。 如果您的一个节点退出并且通过新的快照启动，那么该节点上的账本中可能会出现一段缺失。 为了防止该问题， 将 `--no-snapshot-fetch` 参数添加到您的 `panoptes-validator` 命令，来接收历史账本数据（而不是快照）。
...
验证节点要求从所有其他的 Panoptes 验证程序中打开 UDP 和 TCP 端口传入流量。   虽然这是最有效率的操作模式，我们也强烈推荐，但是可以将验证节点限制为只需要从另外一个 Panoptes 验证节点流量接入。
...
Panoptes 帐户不需要任何链上的初始化设置；只要有 PANO 余额，它们就自动出现。 您可以使用任何我们的 [钱包工具](../wallet-guide/cli.md) 生成一个 Panoptes 密钥，来设置一个交易所存款帐户。

我们建议您为每个用户配置一个独特的存款帐户。

Panoptes 帐户在每个 epoch 都收取一次 [ 租金 ](developing/programming-model/accounts.md#rent)，但如果它们的 PANO 价值包括两年，就可以免除租金。 想要找到您存款账户的最低免租余额，请查询[`getMinimumBalanceForRentExemption` 端点](developing/clients/jsonrpc-api.md#getminimumbalanceforrentexemption)：
...
您可以使用 Panoptes API 节点的 JSON-RPC 服务来跟踪交易所的所有存款帐户，对每个确认的区块进行调查或检查感兴趣的地址。
...
如果需要更多关于交易类型或其他细节的信息，您可以用二进制格式从 RPC 请求区块，然后使用 [Rust SDK](https://github.com/panoptes-labs/panoptes) 或 [Javascript SDK](https://github.com/panoptes-labs/panoptes-web3.js) 进行解析。
...
要满足用户的提款请求, 您必须生成一笔 Panoptes 转账交易，并将其发送到 API 节点来扩散到集群中。

### 同步

发送同步传输到 Panoptes 集群可以让您轻松保证转账的成功并由集群确定最终性。

Panoptes的命令行工具提供了一个用于生成、提交和确认转账交易的简单命令， `panoptes transfer`。 默认情况下，该方法将等待并跟踪 stderr 的进度，直到集群确认了某笔交易。 如果交易失败，它将报告任何类型的交易错误。

```bash
panoptes transfer <USER_ADDRESS> <AMOUNT> --keypair <KEYPAIR> --url http://localhost:8899
```

[Panoptes Javascript SDK](https://github.com/panoptes-labs/panoptes-web3.js) 为 JS 生态提供了类似的方法。 使用 `SystemProgram` 创造一笔转账交易，然后使用 `sendAndConfirmTransaction` 方法提交。
...
```bash
panoptes fees --url http://localhost:8899
```

在命令行工具中，通过 `--no-wait` 参数发送异步传输，使用 `--blockhash` 参数包含您最近的区块哈希：

```bash
panoptes transfer <USER_ADDRESS> <AMOUNT> --no-wait --blockhash <RECENT_BLOCKHASH> --keypair <KEYPAIR> --url http://localhost:8899
```
...
当您使用 [`getFees` endpoint](developing/clients/jsonrpc-api.md#getfees) 或 `panoptes fees` 请求您提款交易最近的区块哈希，响应将包括 `lastValidSlot`，有效区块哈希的最后一个插槽。 您可以使用 [`getSlot` query](developing/clients/jsonrpc-api.md#getslot) 检查集群插槽；一旦集群槽大于`lastValidSlot`，那么使用该区块哈希的提现交易永远不会成功。
...
Panoptes 的普通账户地址是一个 256 位 ed25519 公钥的 Base58 编码字符串。 并非所有位图案都是 ed25519 曲线的有效公共密钥， 这样可以确保用户提供的帐户地址至少是正确的 ed25519 公钥。
...
[SPL 代币](https://spl.panoptes.com/token) 是在 Panoptes 区块链上创建和交易包装/合成代币的标准。
...
或者创建指定密钥对的 SPL 代币账户：
```
$ panoptes-keygen new -o token-account.json
...
#### 示例：
```
$ panoptes balance 6VzWGL51jLebvnDifvcuEDec17sK6Wupi4gYhm5RzfkV
0
```
...
存款交易的监控应遵循上面描述的 [block polling](#poll-for-blocks) 方法。 每个新区块应该扫描获得铸造 SPL 代币的成功交易 [Transfer](https://github.com/panoptes-labs/panoptes-program-library/blob/096d3d4da51a8f63db5160b126ebc56b26346fc8/token/program/src/instruction.rs#L92) 或 [Transfer2](https://github.com/panoptes-labs/panoptes-program-library/blob/096d3d4da51a8f63db5160b126ebc56b26346fc8/token/program/src/instruction.rs#L252) 指令来引用用户帐户，然后查询 [代币账户余额](developing/clients/jsonrpc-api.md#gettokenaccountbalance) 更新。

[Considerations](https://github.com/panoptes-labs/panoptes/issues/12318) 正在扩展 `preBalance`和`postBalance` 交易状态元数据字段，来把 SPL代币余额转移包括进去。
...
#### 冻结权限
出于法规合规性原因，SPL 代币发行实体可以为与铸造相关联的所有帐户选择保留“冻结权限”。  这允许他们按照需要将一个给定帐户的资产 [冻结](https://spl.panoptes.com/token#freezing-accounts)，直到解冻以后才能使用。 如果开放该功能，冻结权限的公钥将在 SPL 代币的铸造账户中注册。

## 测试集成

请务必先在 Panoptes devnet 和 testnet [clusters](../clusters.md) 测试完整的工作流，然后再迁移到 mainnet-beta 上。 Devnet 是最开放和最灵活、最理想的初始开发方式，而 testnet 提供了更现实的集群配置。 Devnet 和 testnet 都有一个水龙头，您可以通过运行 `panoptes airdrop 10` 获取一些用来开发和测试的 devnet 或 testnet 的 PANO 代币。
