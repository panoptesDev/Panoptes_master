...
```bash
panoptes airdrop 1 <RECIPIENT_ACCOUNT_ADDRESS> --url https://api.devnet.panoptes.com
```
...
```bash
panoptes balance <ACCOUNT_ADDRESS> --url https://api.devnet.panoptes.com
```
...
```bash
panoptes-keygen new --no-passphrase --no-outfile
```
...
The Panoptes cluster will only accept the transfer if you sign the transaction
with the private keypair corresponding to the sender's public key in the
transaction.

```bash
panoptes transfer --from <KEYPAIR> <RECIPIENT_ACCOUNT_ADDRESS> 0.5 --allow-unfunded-recipient --url https://api.devnet.panoptes.com --fee-payer <KEYPAIR>
```
...
Confirm the updated balances with `panoptes balance`:

```bash
panoptes balance <ACCOUNT_ADDRESS> --url http://api.devnet.panoptes.com
```
...
```bash
$ panoptes-keygen new --outfile my_panoptes_wallet.json   # Creating my first wallet, a file system wallet
Generating a new keypair
For added security, enter a passphrase (empty for no passphrase):
Wrote new keypair to my_panoptes_wallet.json
==========================================================================
...
$ panoptes airdrop 1 DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.panoptes.com  # Airdropping 1 PANO to my wallet's address/pubkey
Requesting airdrop of 1 PANO from 35.233.193.70:9900
1 PANO

$ panoptes balance DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.panoptes.com # Check the address's balance
1 PANO

$ panoptes-keygen new --no-outfile  # Creating a second wallet, a paper wallet
Generating a new keypair
For added security, enter a passphrase (empty for no passphrase):
====================================================================
...
$ panoptes transfer --from my_panoptes_wallet.json 7S3P4HxJpyyigGzodYwHtCxZyUQe9JiBMHyRWXArAaKv 0.5 --allow-unfunded-recipient --url https://api.devnet.panoptes.com --fee-payer my_panoptes_wallet.json  # Transferring tokens to the public address of the paper wallet
3gmXvykAd1nCQQ7MjosaHLf69Xyaqyq1qw2eu1mgPyYXd5G4v1rihhg1CiRw35b9fHzcftGKKEu4mbUeXY2pEX2z  # This is the transaction signature

$ panoptes balance DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.panoptes.com
...
$ panoptes balance 7S3P4HxJpyyigGzodYwHtCxZyUQe9JiBMHyRWXArAaKv --url https://api.devnet.panoptes.com
...
Panoptes, the wallet address is the public key of a keypair. There are a variety
...
with the `panoptes transfer` command:

```bash
panoptes transfer --from <KEYPAIR> <RECIPIENT_ACCOUNT_ADDRESS> <AMOUNT> --fee-payer <KEYPAIR>
```

Confirm the updated balances with `panoptes balance`:

```bash
panoptes balance <ACCOUNT_ADDRESS>
```
