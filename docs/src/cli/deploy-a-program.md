...
smart contracts elsewhere) with the Panoptes tools.

To learn about developing and executing programs on Panoptes, start with the
...
To deploy a program, use the Panoptes tools to interact with the on-chain loader
to:
...
```bash
panoptes program deploy <PROGRAM_FILEPATH>
```
...
```bash
panoptes program deploy --program-id <KEYPAIR_FILEPATH> <PROGRAM_FILEPATH>
```
...
```bash
panoptes program show <ACCOUNT_ADDRESS>
```
...
```bash
panoptes program deploy --max-len 200000 <PROGRAM_FILEPATH>
```
...
`panoptes-keygen recover` and the following 12-word seed phrase:
==================================================================================
valley flat great hockey share token excess clever benefit traffic avocado athlete
==================================================================================
To resume a deploy, pass the recovered keypair as
the [PROGRAM_ADDRESS_SIGNER] argument to `panoptes deploy` or
as the [BUFFER_SIGNER] to `panoptes program deploy` or `panoptes write-buffer'.
Or to recover the account's lamports, pass it as the
[BUFFER_ACCOUNT_ADDRESS] argument to `panoptes program drain`.
==================================================================================
```
...
```bash
panoptes-keygen recover -o <KEYPAIR_PATH>
```
...
```bash
panoptes program deploy --buffer <KEYPAIR_PATH> <PROGRAM_FILEPATH>
```
...
```bash
panoptes program deploy --upgrade-authority <UPGRADE_AUTHORITY_SIGNER> <PROGRAM_FILEPATH>
```

Or after deployment and using the default keypair as the current authority:

```bash
panoptes program set-upgrade-authority <PROGRAM_ADDRESS> --new-upgrade-authority <NEW_UPGRADE_AUTHORITY>
```

Or after deployment and specifying the current authority:

```bash
panoptes program set-upgrade-authority <PROGRAM_ADDRESS> --upgrade-authority <UPGRADE_AUTHORITY_SIGNER> --new-upgrade-authority <NEW_UPGRADE_AUTHORITY>
```
...
```bash
panoptes program deploy <PROGRAM_FILEPATH> --final
```

Or anytime after:

```bash
panoptes program set-upgrade-authority <PROGRAM_ADDRESS> --final
```

`panoptes program deploy ...` utilizes Panoptes's upgradeable loader, but there is
another way to deploy immutable programs using the original on-chain loader:

```bash
panoptes deploy <PROGRAM_FILEPATH>
```

Programs deployed with `panoptes deploy ...` are not redeployable and are not
compatible with the `panoptes program ...` commands.
...
```bash
panoptes program dump <ACCOUNT_ADDRESS> <OUTPUT_FILEPATH>
```
...
```bash
$ panoptes dump <ACCOUNT_ADDRESS> dump.so
...
```bash
panoptes program write-buffer <PROGRAM_FILEPATH>
```

Buffer accounts support authorities like program accounts:

```bash
panoptes program set-buffer-authority <BUFFER_ADDRESS> --new-upgrade-authority <NEW_UPGRADE_AUTHORITY>
```
...
```bash
panoptes program deploy --program-id <PROGRAM_ADDRESS> --buffer <BUFFER_ADDRESS>
```
