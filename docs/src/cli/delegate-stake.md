...
in a _stake account_. Panoptes weights validator votes by the amount of stake
delegated to them, which gives those validators more influence in determining
then next valid block of transactions in the blockchain. Panoptes then generates
...
View the new stake account with the `panoptes stake-account` command:
...
`panoptes stake-authorize` command. For example, to set a new stake authority,
run:
...
and seed string. To see what stake address the command will derive, use `panoptes create-address-with-seed`:
...
accounts with the `panoptes validators` command:
...
address in `panoptes delegate-stake`:
...
After delegating stake, use `panoptes stake-account` to observe the changes
...
Once delegated, you can undelegate stake with the `panoptes deactivate-stake`
command:
...
Transfer tokens out of a stake account with the `panoptes withdraw-stake` command:
...
existing stake account to a new one, use the `panoptes split-stake` command:
