...
需要注意的是，不支持[getBlockTime](developing/clients/jsonrpc-api.md#getblocktime)，因为一旦https://github.com/Panoptes-labs/Panoptes/issues/10089 被修复，那么`getBlockTime`就可以被删除。
...
## 数据群
通过使用新的`panoptes-ledger-tool`命令，将给定插槽范围的rocksdb数据转换为实例模式，实例数据的持续填充将以一个纪元的节奏进行。
