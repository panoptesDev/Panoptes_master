use {
    ...
    panoptes_config_program::{config_instruction, get_config_data, ConfigState},
    panoptes_sdk::{
        hash::{Hash, Hasher},
        message::Message,
        pubkey::Pubkey,
...
fn load_release_target(release_dir: &Path) -> Result<String, String> {
    ...
    version_yml.push("panoptes-release");
...
pub fn init(
    ...
    let config = {
        // Write new config file only if different, so that running |panoptes-install init|
...
fn github_release_download_url(release_semver: &str) -> String {
    format!(
        "https://github.com/panoptes-labs/panoptes/releases/download/v{}/panoptes-release-{}.tar.bz2",
        release_semver,
...
fn release_channel_download_url(release_channel: &str) -> String {
    format!(
        "http://release.panoptes.com/{}/panoptes-release-{}.tar.bz2",
        release_channel,
        crate::build_env::TARGET
    )
}
...
fn release_channel_version_url(release_channel: &str) -> String {
    format!(
        "http://release.panoptes.com/{}/panoptes-release-{}.yml",
        release_channel,
        crate::build_env::TARGET
    )
}
...
pub fn info(config_file: &str, local_info_only: bool, eval: bool) -> Result<(), String> {
    ...
    if eval {
        println!(
            "PANOPTES_INSTALL_ACTIVE_RELEASE={}",
        ...
        config
            ...
            .and_then(|channel| {
                println!("PANOPTES_INSTALL_ACTIVE_CHANNEL={}", channel,);
                Option::<String>::None
            });
        return Ok(());
    }
...
fn check_for_newer_github_release(
    ...
    let url =
        reqwest::Url::parse("https://api.github.com/repos/panoptes-labs/panoptes/releases").unwrap();
    let client = reqwest::blocking::Client::builder()
        .user_agent("panoptes-install")
        .build()?;
...
pub fn init_or_update(config_file: &str, is_init: bool, check_only: bool) -> Result<bool, String> {
        ...
        match explicit_release {
            ...
            ExplicitRelease::Channel(release_channel) => {
                ...
                let current_release_version_yml =
                    release_dir.join("panoptes-release").join("version.yml");
    ...
    symlink_dir(
        release_dir.join("panoptes-release"),
