#!/usr/bin/env bash
#
# Fetches the latest SPL programs and produces the panoptes-genesis command-line
...
fetch_program() {
  ...
  if [[ -r ~/.cache/panoptes-spl/$so ]]; then
    cp ~/.cache/panoptes-spl/"$so" "$so"
  else
    ...
    (
      ...
      curl -L --retry 5 --retry-delay 2 --retry-connrefused \
        -o "$so" \
        "https://github.com/panoptes-labs/panoptes-program-library/releases/download/$name-v$version/$so_name"
    )

    mkdir -p ~/.cache/panoptes-spl
    cp "$so" ~/.cache/panoptes-spl/"$so"
  fi
...
echo
echo "panoptes-genesis command-line arguments (spl-genesis-args.sh):"
