...
use rayon::prelude::*;
use panoptes_client::perf_utils::{sample_txs, SampleStats};
use panoptes_core::gen_keys::GenKeys;
use panoptes_exchange_program::{exchange_instruction, exchange_state::*, id};
use panoptes_faucet::faucet::request_airdrop_transaction;
    ...
    T: 'static + Client + Send + Sync,
{
    ...
    let s_threads: Vec<_> = (0..threads)
        .map(|_| {
            ...
            Builder::new()
                .name("panoptes-exchange-transfer".to_string())
                .spawn(move || {
    ...
    let swapper_thread = {
        ...
        let client = clients[0].clone();
        Builder::new()
            .name("panoptes-exchange-swapper".to_string())
            .spawn(move || {
                swapper(
                    &exit_signal,
                    &swapper_receiver,
    ...
    let trader_thread = {
        ...
        let client = clients[0].clone();
        Builder::new()
            .name("panoptes-exchange-trader".to_string())
            .spawn(move || {
                trader(
                    &exit_signal,
                    &swapper_sender,
    ...
    let sample_threads: Vec<_> = clients
        ...
        .map(|client| {
            ...
            Builder::new()
                .name("panoptes-exchange-sample".to_string())
