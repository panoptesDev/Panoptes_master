...
case $testName in
test-stable)
  _ "$cargo" stable test --jobs "$NPROC" --all --exclude panoptes-local-cluster ${V:+--verbose} -- --nocapture
  ;;
test-stable-perf)
  # panoptes-keygen required when building C programs
  _ "$cargo" build --manifest-path=keygen/Cargo.toml
  export PATH="$PWD/target/debug":$PATH

  # BPF panoptes-sdk legacy compile test
  ...
  if [[ $(uname) = Linux ]]; then
    ...
    # Force CUDA for panoptes-core unit tests
  ...
  _ "$cargo" stable test --package panoptes-perf --package panoptes-ledger --package panoptes-core --lib ${V:+--verbose} -- --nocapture
  _ "$cargo" stable run --manifest-path poh-bench/Cargo.toml ${V:+--verbose} -- --hashes-per-tick 10
  ;;
test-local-cluster)
  _ "$cargo" stable build --release --bins ${V:+--verbose}
  _ "$cargo" stable test --release --package panoptes-local-cluster ${V:+--verbose} -- --nocapture --test-threads=1
  exit 0
  ;;
*)
