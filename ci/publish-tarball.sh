...
RELEASE_BASENAME="${RELEASE_BASENAME:=panoptes-release}"
...
(
  ...
  cp "${RELEASE_BASENAME}"/bin/panoptes-install-init panoptes-install-init-$TARGET
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml panoptes-install-init-"$TARGET"* $MAYBE_TARBALLS; do
  ...
  if [[ -n $BUILDKITE ]]; then
    ...
    upload-s3-artifact "/panoptes/$file" s3://release.panoptes.com/"$CHANNEL_OR_TAG"/"$file"

    echo Published to:
    $DRYRUN ci/format-url.sh https://release.panoptes.com/"$CHANNEL_OR_TAG"/"$file"

    if [[ -n $TAG ]]; then
      ci/upload-github-release-asset.sh "$file"
    fi
  elif [[ -n $TRAVIS ]]; then
    # .travis.yml uploads everything in the travis-s3-upload/ directory to release.panoptes.com
...
# Create install wrapper for release.panoptes.com
if [[ -n $DO_NOT_PUBLISH_TAR ]]; then
  echo "Skipping publishing install wrapper"
elif [[ -n $BUILDKITE ]]; then
  cat > release.panoptes.com-install <<EOF
PANOPTES_RELEASE=$CHANNEL_OR_TAG
PANOPTES_INSTALL_INIT_ARGS=$CHANNEL_OR_TAG
PANOPTES_DOWNLOAD_ROOT=http://release.panoptes.com
EOF
  cat install/panoptes-install-init.sh >> release.panoptes.com-install

  echo --- AWS S3 Store: "install"
  $DRYRUN upload-s3-artifact "/panoptes/release.panoptes.com-install" "s3://release.panoptes.com/$CHANNEL_OR_TAG/install"
  echo Published to:
  $DRYRUN ci/format-url.sh https://release.panoptes.com/"$CHANNEL_OR_TAG"/install
fi
