...
#[macro_use]
extern crate panoptes_bpf_loader_program;

use itertools::izip;
use log::{log_enabled, trace, Level::Trace};
use panoptes_account_decoder::parse_bpf_loader::{
...
use panoptes_cli_output::display::println_transaction;
use panoptes_rbpf::{
    static_analysis::Analysis,
    vm::{Config, Executable, Tracer},
};
use panoptes_runtime::{
...
use panoptes_transaction_status::{
...
fn write_bpf_program(
    ...
    use panoptes_sdk::loader_instruction;
...
fn test_program_bpf_sanity() {
    ...
    {
        programs.extend_from_slice(&[
            ("panoptes_bpf_rust_128bit", true),
            ("panoptes_bpf_rust_alloc", true),
            ("panoptes_bpf_rust_custom_heap", true),
            ("panoptes_bpf_rust_dep_crate", true),
            ("panoptes_bpf_rust_external_spend", false),
            ("panoptes_bpf_rust_iter", true),
            ("panoptes_bpf_rust_many_args", true),
            ("panoptes_bpf_rust_membuiltins", true),
            ("panoptes_bpf_rust_noop", true),
            ("panoptes_bpf_rust_panic", false),
            ("panoptes_bpf_rust_param_passing", true),
            ("panoptes_bpf_rust_rand", true),
            ("panoptes_bpf_rust_sanity", true),
            ("panoptes_bpf_rust_sha", true),
        ]);
    }
    ...
    for program in programs.iter() {
        ...
        let (name, id, entrypoint) = panoptes_bpf_loader_program!();
...
fn test_program_bpf_loader_deprecated() {
    ...
    #[cfg(feature = "bpf_rust")]
    {
        programs.extend_from_slice(&[("panoptes_bpf_rust_deprecated_loader")]);
    }
    ...
    for program in programs.iter() {
        ...
        let mut bank = Bank::new(&genesis_config);
        let (name, id, entrypoint) = panoptes_bpf_loader_deprecated_program!();
        bank.add_builtin(&name, id, entrypoint);
        let bank_client = BankClient::new(bank);
...
fn test_program_bpf_duplicate_accounts() {
    ...
    #[cfg(feature = "bpf_rust")]
    {
        programs.extend_from_slice(&[("panoptes_bpf_rust_dup_accounts")]);
    }
    ...
    for program in programs.iter() {
        ...
        let payee_pubkey = panoptes_sdk::pubkey::new_rand();
...
fn test_program_bpf_error_handling() {
    ...
    #[cfg(feature = "bpf_rust")]
    {
        programs.extend_from_slice(&[("panoptes_bpf_rust_error_handling")]);
    }
...
fn test_program_bpf_invoke_sanity() {
    ...
    #[cfg(feature = "bpf_rust")]
    {
        programs.push((
            Languages::Rust,
            "panoptes_bpf_rust_invoke",
            "panoptes_bpf_rust_invoked",
            "panoptes_bpf_rust_noop",
        ));
    }
    for program in programs.iter() {
        println!("Test program: {:?}", program);
        ...
        let account = AccountSharedData::new(84, 0, &panoptes_sdk::system_program::id());
        ...
        let account_metas = vec![
            ...
            AccountMeta::new_readonly(panoptes_sdk::system_program::id(), false),
        ...
        let expected_invoked_programs = match program.0 {
            Languages::C => vec![
                panoptes_sdk::system_program::id(),
                panoptes_sdk::system_program::id(),
        ...
        let invoked_programs: Vec<Pubkey> = inner_instructions[0]
            ...
            .map(|ix| message.account_keys[ix.program_id_index as usize].clone())
            .collect();
        assert_eq!(invoked_programs, vec![panoptes_sdk::system_program::id()]);
        assert_eq!(
            result.unwrap_err(),
            TransactionError::InstructionError(0, InstructionError::ProgramFailedToComplete)
        );
...
fn test_program_bpf_program_id_spoofing() {
    ...
    let malicious_swap_pubkey = load_bpf_program(
        ...
        &bpf_loader::id(),
        &mint_keypair,
        "panoptes_bpf_rust_spoof1",
    );
    let malicious_system_pubkey = load_bpf_program(
        &bank_client,
        &bpf_loader::id(),
        &mint_keypair,
        "panoptes_bpf_rust_spoof1_system",
    );

    let from_pubkey = Pubkey::new_unique();
    let account = AccountSharedData::new(10, 0, &panoptes_sdk::system_program::id());
    bank.store_account(&from_pubkey, &account);

    let to_pubkey = Pubkey::new_unique();
    let account = AccountSharedData::new(0, 0, &panoptes_sdk::system_program::id());
...
fn test_program_bpf_caller_has_access_to_cpi_program() {
    ...
    let caller_pubkey = load_bpf_program(
        ...
        &bpf_loader::id(),
        &mint_keypair,
        "panoptes_bpf_rust_caller_access",
    );
...
fn test_program_bpf_call_depth() {
    use panoptes_sdk::process_instruction::BpfComputeBudget;

    panoptes_logger::setup();

    println!("Test program: panoptes_bpf_rust_call_depth");
...
fn assert_instruction_count() {
    ...
    {
        programs.extend_from_slice(&[
            ("panoptes_bpf_rust_128bit", 572),
            ("panoptes_bpf_rust_alloc", 8906),
            ("panoptes_bpf_rust_custom_heap", 539),
            ("panoptes_bpf_rust_dep_crate", 2),
            ("panoptes_bpf_rust_external_spend", 521),
            ("panoptes_bpf_rust_iter", 724),
            ("panoptes_bpf_rust_many_args", 237),
            ("panoptes_bpf_rust_mem", 3166),
            ("panoptes_bpf_rust_membuiltins", 4069),
            ("panoptes_bpf_rust_noop", 495),
            ("panoptes_bpf_rust_param_passing", 46),
            ("panoptes_bpf_rust_rand", 498),
            ("panoptes_bpf_rust_sanity", 917),
            ("panoptes_bpf_rust_sha", 29099),
        ]);
    }
...
fn test_program_bpf_instruction_introspection() {
    ...
    let program_id = load_bpf_program(
        ...
        &bpf_loader::id(),
        &mint_keypair,
        "panoptes_bpf_rust_instruction_introspection",
    );

    // Passing transaction
    let account_metas = vec![AccountMeta::new_readonly(
        panoptes_sdk::sysvar::instructions::id(),
        false,
    )];
    ...
    assert_eq!(
        ...
        TransactionError::InstructionError(
            0,
            panoptes_sdk::instruction::InstructionError::NotEnoughAccountKeys
        )
    );
    assert!(bank
        .get_account(&panoptes_sdk::sysvar::instructions::id())
        .is_none());
}
...
fn test_program_bpf_test_use_latest_executor() {
    use panoptes_sdk::{loader_instruction, system_instruction};
    ...
    let panic_id = load_bpf_program(
        ...
        &bpf_loader::id(),
        &mint_keypair,
        "panoptes_bpf_rust_panic",
    );

    let program_keypair = Keypair::new();

    // Write the panic program into the program account
    let elf = read_bpf_program("panoptes_bpf_rust_panic");
    let message = Message::new(
    ...
    let elf = read_bpf_program("panoptes_bpf_rust_noop");
    write_bpf_program(
        &bank_client,
        &bpf_loader::id(),
        &mint_keypair,
...
fn test_program_bpf_test_use_latest_executor2() {
    ...
    let invoke_and_error = load_bpf_program(
        ...
        &bpf_loader::id(),
        &mint_keypair,
        "panoptes_bpf_rust_invoke_and_error",
    );
    let invoke_and_ok = load_bpf_program(
        &bank_client,
        &bpf_loader::id(),
        &mint_keypair,
        "panoptes_bpf_rust_invoke_and_ok",
    );
...
fn test_program_bpf_upgrade() {
    ...
    let mut bank = Bank::new(&genesis_config);
    let (name, id, entrypoint) = panoptes_bpf_loader_upgradeable_program!();
    bank.add_builtin(&name, id, entrypoint);
    let bank_client = BankClient::new(bank);
...
fn test_program_bpf_invoke_upgradeable_via_cpi() {
    ...
    let invoke_and_return = load_bpf_program(
        ...
        &bpf_loader::id(),
        &mint_keypair,
        "panoptes_bpf_rust_invoke_and_return",
    );
...
fn test_program_bpf_disguised_as_bpf_loader() {
    ...
    #[cfg(feature = "bpf_rust")]
    {
        programs.extend_from_slice(&[("panoptes_bpf_rust_noop")]);
    }
...
fn test_program_bpf_c_dup() {
    ...
    let account =
        AccountSharedData::new_data(42, &[1_u8, 2, 3], &panoptes_sdk::system_program::id()).unwrap();
...
fn test_program_bpf_upgrade_via_cpi() {
    ...
    let path = create_bpf_path("panoptes_bpf_rust_upgraded");
    let mut file = File::open(&path).unwrap_or_else(|err| {
        panic!("Failed to open {}: {}", path.display(), err);
    });
    let mut elf = Vec::new();
...
fn test_program_upgradeable_locks() {
    fn setup_program_upgradeable_locks(
        ...
        // Load the buffer account
        let path = create_bpf_path("panoptes_bpf_rust_noop");
        let mut file = File::open(&path).unwrap_or_else(|err| {
            panic!("Failed to open {}: {}", path.display(), err);
        });
        let mut elf = Vec::new();
...
fn test_program_bpf_ro_account_modify() {
    ...
    let program_id = load_bpf_program(
        ...
        &bpf_loader::id(),
        &mint_keypair,
        "panoptes_bpf_rust_ro_account_modify",
    );
