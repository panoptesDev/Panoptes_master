...
use panoptes_rbpf::vm::{Config, Executable, InstructionMeter};
...
fn bench_instruction_count_tuner(_bencher: &mut Bencher) {
    ...
    let keys = [panoptes_sdk::pubkey::new_rand()];
    let keyed_accounts: Vec<_> = keys
        .iter()
        .zip(&accounts)
        .map(|(key, account)| panoptes_sdk::keyed_account::KeyedAccount::new(&key, false, &account))
        .collect();
