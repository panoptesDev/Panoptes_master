...
#[deprecated(
    since = "1.7.2",
    note = "Please use `panoptes_sdk::stake::instruction` or `panoptes_program::stake::instruction` instead"
)]
pub use panoptes_sdk::stake::instruction::*;
...
mod tests {
        ...
        let accounts: Vec<_> = instruction
            ...
            .map(|meta| {
                RefCell::new(if sysvar::clock::check_id(&meta.pubkey) {
                    ...
                    AccountSharedData::from(Account {
                        owner: panoptes_vote_program::id(),
                        ..Account::default()
                    })
    ...
    fn test_stake_process_instruction_decode_bail() {
        ...
        bad_vote_account
            .get_mut()
            .set_owner(panoptes_vote_program::id());
