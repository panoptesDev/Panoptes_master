...
mod tests {
    ...
    fn test_ownable_missing_owner_signature() {
        let mut account_owner_pubkey = panoptes_sdk::pubkey::new_rand();
        let owner_pubkey = account_owner_pubkey;
        let new_owner_pubkey = panoptes_sdk::pubkey::new_rand();
    ...
    fn test_ownable_incorrect_owner() {
        ...
        let new_owner_pubkey = panoptes_sdk::pubkey::new_rand();
        let account = AccountSharedData::new_ref(1, 0, &system_program::id());
        let mallory_pubkey = panoptes_sdk::pubkey::new_rand(); // <-- Attack! Signing with wrong pubkey
