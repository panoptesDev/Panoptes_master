...
use panoptes_download_utils::download_snapshot;
use panoptes_gossip::{
...
const RUST_LOG_FILTER: &str =
    "error,panoptes_core::replay_stage=warn,panoptes_local_cluster=info,local_cluster=info";

#[test]
#[serial]
fn test_ledger_cleanup_service() {
    panoptes_logger::setup_with_default(RUST_LOG_FILTER);
...
fn test_mainnet_beta_cluster_type() {
    ...
    for program_id in [
        &panoptes_config_program::id(),
        &panoptes_sdk::system_program::id(),
        &panoptes_sdk::stake::program::id(),
    ...
    for program_id in [&panoptes_sdk::bpf_loader_upgradeable::id()].iter() {
        assert_eq!(
            (
                program_id,
                client
...
fn generate_frozen_account_panic(mut cluster: LocalCluster, frozen_account: Arc<Keypair>) {
    ...
    panoptes_runtime::accounts_db::FROZEN_ACCOUNT_PANIC.store(false, Ordering::Relaxed);

    // Wait for the frozen account panic signal
    let mut i = 0;
    while !panoptes_runtime::accounts_db::FROZEN_ACCOUNT_PANIC.load(Ordering::Relaxed) {
...
fn test_frozen_account_from_genesis() {
    ...
    let validator_identity =
        Arc::new(panoptes_sdk::signature::keypair_from_seed(&[0u8; 32]).unwrap());
...
fn test_faulty_node(faulty_node_type: BroadcastStageType) {
    panoptes_logger::setup_with_default("panoptes_local_cluster=info");
    let num_nodes = 3;
...
fn test_hard_fork_invalidates_tower() {
    ...
    let mut hard_forks = panoptes_sdk::hard_forks::HardForks::default();
    hard_forks.register(hard_fork_slot);

    let expected_shred_version = panoptes_sdk::shred_version::compute_shred_version(
