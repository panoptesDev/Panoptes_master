...
use {
    ...
    rand::{thread_rng, Rng},
    panoptes_client::thin_client::{create_client, ThinClient},
    panoptes_perf::recycler::Recycler,
    panoptes_runtime::bank_forks::BankForks,
    panoptes_sdk::{
        pubkey::Pubkey,
        signature::{Keypair, Signer},
    },
    panoptes_streamer::streamer,
    std::{
        collections::HashSet,
...
pub fn discover(
    ...
    let _ip_echo_server = ip_echo
        .map(|tcp_listener| panoptes_net_utils::ip_echo_server(tcp_listener, Some(my_shred_version)));
...
pub fn get_multi_client(nodes: &[ContactInfo]) -> (ThinClient, usize) {
    ...
    let (_, transactions_socket) = panoptes_net_utils::bind_in_range(
...
mod tests {
    ...
    fn test_gossip_services_spy() {
        ...
        let peer0 = panoptes_sdk::pubkey::new_rand();
        let peer1 = panoptes_sdk::pubkey::new_rand();
        ...
        let (met_criteria, _, _, _) = spy(
            ...
            None,
            TIMEOUT,
            Some(panoptes_sdk::pubkey::new_rand()),
            None,
        );
        assert!(!met_criteria);
