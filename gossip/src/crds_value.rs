use {
    ...
    panoptes_sdk::sanitize::{Sanitize, SanitizeError},
    ...
    panoptes_vote_program::vote_transaction::parse_vote_transaction,
    std::{
        borrow::{Borrow, Cow},
        cmp::Ordering,
...
impl SnapshotHash {
    ...
    pub(crate) fn new_rand<R: Rng>(rng: &mut R, pubkey: Option<Pubkey>) -> Self {
        ...
        let hashes = std::iter::repeat_with(|| {
            ...
            let hash = panoptes_sdk::hash::new_rand(rng);
            (slot, hash)
        })
        .take(num_hashes)
        .collect();
...
pub struct LegacyVersion {
    pub from: Pubkey,
    pub wallclock: u64,
    pub version: panoptes_version::LegacyVersion,
}
...
pub struct Version {
    pub from: Pubkey,
    pub wallclock: u64,
    pub version: panoptes_version::Version,
}
...
impl Version {
    pub fn new(from: Pubkey) -> Self {
        Self {
            from,
            wallclock: timestamp(),
            version: panoptes_version::Version::default(),
        }
    }
...
mod test {
    use {
        ...
        panoptes_vote_program::{vote_instruction, vote_state},
