...
use {
    ...
    panoptes_sdk::{packet::PACKET_DATA_SIZE, pubkey::Pubkey, timing::timestamp},
    std::{
        cmp,
...
mod test {
    ...
    fn test_prune() {
        ...
        let self_id = panoptes_sdk::pubkey::new_rand();
        let origin = panoptes_sdk::pubkey::new_rand();
        ...
        let low_staked_peers = (0..10).map(|_| panoptes_sdk::pubkey::new_rand());
        ...
        let high_staked_peer = panoptes_sdk::pubkey::new_rand();
    ...
    fn test_process_push_old_version() {
        let mut crds = Crds::default();
        let mut push = CrdsGossipPush::default();
        let mut ci = ContactInfo::new_localhost(&panoptes_sdk::pubkey::new_rand(), 0);
        ci.wallclock = 1;
    ...
    fn test_refresh_active_set() {
        ...
        for _ in 0..push.num_active {
            let value2 = CrdsValue::new_unsigned(CrdsData::ContactInfo(
                ContactInfo::new_localhost(&panoptes_sdk::pubkey::new_rand(), 0),
            ));
            assert_eq!(crds.insert(value2.clone(), now), Ok(()));
        }
        push.refresh_push_active_set(&crds, &HashMap::new(), None, &Pubkey::default(), 0, 1, 1);
