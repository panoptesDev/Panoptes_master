...
use {
    ...
    panoptes_cli_output::display::format_labeled_address,
    panoptes_client::{client_error, rpc_client::RpcClient, rpc_response::RpcVoteAccountStatus},
    panoptes_metrics::{datapoint_error, datapoint_info},
    panoptes_notifier::Notifier,
    panoptes_sdk::{
        hash::Hash,
...
fn get_config() -> Config {
    let matches = App::new(crate_name!())
        ...
        define environment variables before running `panoptes-watchtower`:
        ...
        define environment variable before running `panoptes-watchtower`:
        ...
        .arg(
            Arg::with_name("minimum_validator_identity_balance")
                ...
                .help("Alert when the validator identity balance is less than this amount of PANO")
        )
        .arg(
...
fn main() -> Result<(), Box<dyn error::Error>> {
    panoptes_logger::setup_with_default("panoptes=info");
    panoptes_metrics::set_panic_hook("watchtower");
    ...
    loop {
        ...
        if let Some((failure_test_name, failure_error_message)) = &failure {
            ...
            if !last_notification_msg.is_empty() {
                ...
                info!("{}", all_clear_msg);
                notifier.send(&format!("panoptes-watchtower: {}", all_clear_msg));
            }
