...
use panoptes_metrics::{self, datapoint_info};
use panoptes_sdk::{
    client::Client,
    ...
    T: 'static + Client + Send + Sync,
{
    ...
    Builder::new()
        .name("panoptes-client-sample".to_string())
        .spawn(move || {
    ...
    T: 'static + Client + Send + Sync,
{
    (0..threads)
        .map(|_| {
            ...
            let client = client.clone();
            Builder::new()
                .name("panoptes-client-sender".to_string())
                .spawn(move || {
                    do_tx_transfers(
                        &exit_signal,
                        &shared_txs,
    ...
    T: 'static + Client + Send + Sync,
{
    ...
    let blockhash_thread = {
        ...
        let id = id.pubkey();
        Builder::new()
            .name("panoptes-blockhash-poller".to_string())
            .spawn(move || {
...
pub fn generate_and_fund_keypairs<T: 'static + Client + Send + Sync>(
    ...
    // Sample the first keypair, to prevent lamport loss on repeated panoptes-bench-tps executions
