...
if [[ $CI_OS_NAME = windows ]]; then
  ...
  BINS=(
    ...
    panoptes-install
    panoptes-keygen
    panoptes-ledger-tool
    panoptes-log-analyzer
    panoptes-net-shaper
    panoptes-sys-tuner
    panoptes-validator
  )
  ...
  #XXX: Ensure `panoptes-genesis` is built LAST!
  # See https://github.com/panoptes-labs/panoptes/issues/5826
  BINS+=(panoptes-genesis)
fi
...
(
  ...
  shopt -s nullglob
  for dep in target/"$buildVariant"/deps/libpanoptes*program.*; do
