...
function analyze_packet_loss {
  (
    ...
    for ip in "${validatorIpList[@]}"; do
      "${REPO_ROOT}"/net/scp.sh ip_address_map.txt panoptes@"$ip":~/panoptes/
    done
    ...
    for ip in "${validatorIpList[@]}"; do
      ...
      "${REPO_ROOT}"/net/ssh.sh panoptes@"$ip" 'PATH=$PATH:~/.cargo/bin/ ~/panoptes/scripts/iftop-postprocess.sh ~/panoptes/iftop.log temp.log ~panoptes/panoptes/ip_address_map.txt' > "$iftop_log"
      upload-ci-artifact "$iftop_log"
    done

    execution_step "Analyzing Packet Loss"
    "${REPO_ROOT}"/panoptes-release/bin/panoptes-log-analyzer analyze -f ./iftop-logs/ | sort -k 2 -g
  )
}
...
function wait_for_bootstrap_validator_stake_drop {
  ...
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" "RUST_LOG=info \$HOME/.cargo/bin/panoptes wait-for-max-stake $max_stake --url http://127.0.0.1:8899"
}

function get_slot {
  source "${REPO_ROOT}"/net/common.sh
  loadConfigFile
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" '$HOME/.cargo/bin/panoptes --url http://127.0.0.1:8899 slot'
}
...
function upload_results_to_slack() {
  ...
  COMMIT_URL="https://github.com/panoptes-labs/panoptes/commit/${COMMIT}"

  if [[ -n $BUILDKITE_BUILD_URL ]] ; then
    BUILD_BUTTON_TEXT="Build Kite Job"
  else
    BUILD_BUTTON_TEXT="Build URL not defined"
    BUILDKITE_BUILD_URL="https://buildkite.com/panoptes-labs/"
  fi

  GRAFANA_URL="https://metrics.panoptes.com:3000/d/monitor-${CHANNEL:-edge}/cluster-telemetry-${CHANNEL:-edge}?var-testnet=${TESTNET_TAG:-testnet-automation}&from=${TESTNET_START_UNIX_MSECS:-0}&to=${TESTNET_FINISH_UNIX_MSECS:-0}"
