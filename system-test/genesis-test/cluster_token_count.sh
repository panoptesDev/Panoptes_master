...
usage: $0 [cluster_rpc_url]
 ...
 Required arguments:
   cluster_rpc_url  - RPC URL and port for a running Panoptes cluster (ex: http://34.83.146.144:8899)
EOF
  exit $exitcode
}

function get_cluster_version {
  clusterVersion="$(curl -s -X POST -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","id":1, "method":"getVersion"}' "$url" | jq '.result | ."panoptes-core" ')"
...
function get_token_capitalization {
  ...
  totalSupplySol=$((totalSupplyLamports / LAMPORTS_PER_PANO))

  printf "\n--- Token Capitalization ---\n"
  printf "Total token capitalization %'d PANO\n" "$totalSupplySol"
...
function sum_account_balances_totals {
  ...
  printf "Total PANO in all Account Balances: %'d\n" "$grandTotalAccountBalancesSol"
...
LAMPORTS_PER_PANO=1000000000 # 1 billion
