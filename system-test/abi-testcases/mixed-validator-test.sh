...
for v in "${otherVersions[@]}"; do
  panoptes-install-init "${panoptesInstallGlobalOpts[@]}" "$v"
  panoptes -V
done
...
panoptesInstallUse() {
  declare version=$1
  echo "--- Now using panoptes $version"
  PANOPTES_BIN="$panoptesInstallDataDir/releases/$version/panoptes-release/bin"
  export PATH="$PANOPTES_BIN:$ORIGINAL_PATH"
}
...
panoptesInstallUse "$baselineVersion"
...
(
  ...
  if [[ ! -x baseline-run.sh ]]; then
    curl https://raw.githubusercontent.com/panoptes-labs/panoptes/v"$baselineVersion"/run.sh -o baseline-run.sh
  ...
  panoptes --url http://127.0.0.1:8899 show-validators
)

# Ensure all versions can see the bootstrap validator
for v in "${otherVersions[@]}"; do
  panoptesInstallUse "$v"
  echo "--- Looking for bootstrap validator on gossip"
  (
    set -x
    "$PANOPTES_BIN"/panoptes-gossip spy \
...
#
# Once https://github.com/panoptes-labs/panoptes/issues/7738 is resolved, remove
...
for v in "${otherVersions[@]}"; do
  ...
  (
    ...
    tmux new-window -t abi -n "$v" " \
      $PANOPTES_BIN/panoptes-validator \
      --ledger $ledger \
      --no-snapshot-fetch \
      --entrypoint 127.0.0.1:8001 \
