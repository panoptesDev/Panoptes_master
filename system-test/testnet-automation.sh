...
function launch_testnet() {
  ...
  if [[ ! -d "${REPO_ROOT}"/net/keypairs ]]; then
    git clone git@github.com:panoptes-labs/testnet-keypairs.git "${REPO_ROOT}"/net/keypairs
  ...
  get_net_launch_software_version_launch_args "$CHANNEL" "panoptes-release" version_args
...
[[ -n $INFLUX_HOST ]] || INFLUX_HOST=https://metrics.panoptes.com:8086
...
if [[ -z $PANOPTES_METRICS_CONFIG ]]; then
  if [[ -z $PANOPTES_METRICS_PARTIAL_CONFIG ]]; then
    echo PANOPTES_METRICS_PARTIAL_CONFIG not defined
    exit 1
  fi
  export PANOPTES_METRICS_CONFIG="db=$TESTNET_TAG,host=$INFLUX_HOST,$PANOPTES_METRICS_PARTIAL_CONFIG"
fi
echo "PANOPTES_METRICS_CONFIG: $PANOPTES_METRICS_CONFIG"
