...
use {
    ...
    panoptes_download_utils::{download_snapshot, DownloadProgressRecord},
    panoptes_genesis_utils::download_then_check_genesis_hash,
    panoptes_gossip::{
        cluster_info::{ClusterInfo, Node, VALIDATOR_PORT_RANGE},
        contact_info::ContactInfo,
        gossip_service::GossipService,
    },
    panoptes_ledger::blockstore_db::BlockstoreRecoveryMode,
    panoptes_perf::recycler::enable_recycler_warming,
    panoptes_poh::poh_service,
    panoptes_rpc::{rpc::JsonRpcConfig, rpc_pubsub_service::PubSubConfig},
    panoptes_runtime::{
        accounts_db::{
...
fn check_vote_account(
    ...
    if vote_account.owner != panoptes_vote_program::id() {
        return Err(format!(
    ...
    let vote_state = panoptes_vote_program::vote_state::VoteState::from(&vote_account);
...
fn verify_reachable_ports(
    ...
    panoptes_net_utils::verify_reachable_ports(
...
fn rpc_bootstrap(
    ...
    loop {
        ...
        let result = match rpc_client.get_version() {
            Ok(rpc_version) => {
                info!("RPC node version: {}", rpc_version.panoptes_core);
                Ok(())
            }
        ...
        .map(|_| {
            if !validator_config.voting_disabled && !bootstrap_config.no_check_vote_account {
                ...
                .unwrap_or_else(|err| {
                    ...
                    // incorrect `panoptes-validator` command-line arguments) rather than the
                    // RPC node failing.
                    //
...
fn get_cluster_shred_version(entrypoints: &[SocketAddr]) -> Option<u16> {
    ...
    for entrypoint in entrypoints {
        match panoptes_net_utils::get_cluster_shred_version(entrypoint) {
...
pub fn main() {
    ...
    let matches = App::new(crate_name!()).about(crate_description!())
        ...
        .arg(
            Arg::with_name("entrypoint")
                ...
                .takes_value(true)
                .multiple(true)
                .validator(panoptes_net_utils::is_host_port)
        ...
        .arg(
            Arg::with_name("rpc_port")
                ...
                .value_name("PORT")
                .takes_value(true)
                .validator(panoptes_validator::port_validator)
        ...
        .arg(
            Arg::with_name("gossip_host")
                ...
                .value_name("HOST")
                .takes_value(true)
                .validator(panoptes_net_utils::is_host)
        ...
        .arg(
            Arg::with_name("dynamic_port_range")
                ...
                .validator(panoptes_validator::port_range_validator)
        ...
        .arg(
            Arg::with_name("snapshot_archive_format")
                ...
                .alias("snapshot-compression") // Legacy name used by Panoptes v1.5.x and older
    ...
    let bind_address = panoptes_net_utils::parse_host(matches.value_of("bind_address").unwrap())
        .expect("invalid bind_address");
    let rpc_bind_address = if matches.is_present("rpc_bind_address") {
        panoptes_net_utils::parse_host(matches.value_of("rpc_bind_address").unwrap())
    ...
    let entrypoint_addrs = values_t!(matches, "entrypoint", String)
        ...
        .into_iter()
        .map(|entrypoint| {
            panoptes_net_utils::parse_host_port(&entrypoint).unwrap_or_else(|e| {
    ...
    let mut validator_config = ValidatorConfig {
        ...
        rpc_config: JsonRpcConfig {
            ...
            faucet_addr: matches.value_of("rpc_faucet_addr").map(|address| {
                panoptes_net_utils::parse_host_port(address).expect("failed to parse faucet address")
            }),
        ...
        rpc_addrs: value_t!(matches, "rpc_port", u16).ok().map(|rpc_port| {
            (
                ...
                // https://github.com/panoptes-labs/panoptes/issues/12250
            )
        }),
    ...
    let dynamic_port_range =
        panoptes_net_utils::parse_port_range(matches.value_of("dynamic_port_range").unwrap())
    ...
    let public_rpc_addr = matches.value_of("public_rpc_addr").map(|addr| {
        panoptes_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
    ...
    let logfile = {
        let logfile = matches
            .value_of("logfile")
            .map(|s| s.into())
            .unwrap_or_else(|| format!("panoptes-validator-{}.log", identity_keypair.pubkey()));
    ...
    info!("{} {}", crate_name!(), panoptes_version::version!());
    ...
    let gossip_host: IpAddr = matches
        ...
        .map(|gossip_host| {
            panoptes_net_utils::parse_host(gossip_host).unwrap_or_else(|err| {
        ...
        .unwrap_or_else(|| {
            if !entrypoint_addrs.is_empty() {
                ...
                let gossip_host = order.into_iter().find_map(|i| {
                    ...
                    panoptes_net_utils::get_public_ip_addr(entrypoint_addr).map_or_else(
                        |err| {
                            eprintln!(
    ...
    let gossip_addr = SocketAddr::new(
        ...
        value_t!(matches, "gossip_port", u16).unwrap_or_else(|_| {
            panoptes_net_utils::find_available_port_in_range(bind_address, (0, 1)).unwrap_or_else(
                |err| {
    ...
    panoptes_metrics::set_host_id(identity_keypair.pubkey().to_string());
    panoptes_metrics::set_panic_hook("validator");

    if validator_config.cuda {
        panoptes_perf::perf_libs::init_cuda();
        enable_recycler_warming();
    }
    panoptes_ledger::entry::init_poh();
    panoptes_runtime::snapshot_utils::remove_tmp_snapshot_archives(&snapshot_output_dir);
