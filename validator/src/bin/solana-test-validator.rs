use {
    ...
    panoptes_client::rpc_client::RpcClient,
    panoptes_faucet::faucet::{run_local_faucet_with_port, FAUCET_PORT},
    panoptes_rpc::rpc::JsonRpcConfig,
    panoptes_sdk::{
 ...
 * 40MB-150MB range when running `panoptes-test-validator`
 */
...
fn main() {
    ...
    let matches = App::new("panoptes-test-validator")
        ...
        .arg({
            ...
            if let Some(ref config_file) = *panoptes_cli_config::CONFIG_FILE {
        ...
        .arg(
            Arg::with_name("json_rpc_url")
                ...
                .help(
                    "URL for Panoptes's JSON RPC or moniker (or their first letter): \
    ...
    let cli_config = if let Some(config_file) = matches.value_of("config_file") {
        panoptes_cli_config::Config::load(config_file).unwrap_or_default()
    } else {
        panoptes_cli_config::Config::default()
    };
    ...
    let dynamic_port_range = matches.value_of("dynamic_port_range").map(|port_range| {
        panoptes_net_utils::parse_port_range(port_range).unwrap_or_else(|| {
            eprintln!("Failed to parse --dynamic-port-range");
            exit(1);
        })
    });
    let bind_address = matches.value_of("bind_address").map(|bind_address| {
        panoptes_net_utils::parse_host(bind_address).unwrap_or_else(|err| {
            ...
            match address_program {
                [address, program] => {
                    ...
                    programs.push(ProgramInfo {
                        program_id: address,
                        loader: panoptes_sdk::bpf_loader::id(),
                        program_path,
                    });
                }
                _ => unreachable!(),
    ...
    panoptes_runtime::snapshot_utils::remove_tmp_snapshot_archives(&ledger_path);
    ...
    if TestValidatorGenesis::ledger_exists(&ledger_path) {
        ...
        println_name_value(
            "\nNotice!",
            "No wallet available. `panoptes airdrop` localnet PANO after creating one\n",
        );
    }
