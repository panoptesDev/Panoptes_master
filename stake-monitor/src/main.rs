...
use panoptes_metrics::datapoint_error;
use panoptes_sdk::{clock::Slot, native_token::lamports_to_sol, pubkey::Pubkey, system_program};
use panoptes_stake_monitor::*;
use std::{fs, io, process};

fn load_accounts_info(data_file: &str) -> AccountsInfo {
    let data_file_new = data_file.to_owned() + "new";
    let accounts_info = panoptes_cli_config::load_config_file(&data_file_new)
        .or_else(|_| panoptes_cli_config::load_config_file(data_file))
        .unwrap_or_default();
...
fn save_accounts_info(data_file: &str, accounts_info: &AccountsInfo) -> io::Result<()> {
    let data_file_new = data_file.to_owned() + "new";
    panoptes_cli_config::save_config_file(&accounts_info, &data_file_new)?;
...
fn command_enroll(data_file: &str, json_rpc_url: String, account_address: &Pubkey) {
    ...
    println!(
        "Enrolled {} at slot {} with a balance of {} PANO",
        account_address,
        slot,
...
fn command_check(data_file: &str, account_address: &Pubkey) {
    ...
    if let Some(account_info) = accounts_info.account_info.get(&account_address.to_string()) {
        if let Some(slot) = account_info.compliant_since {
            println!(
                "{}Account compliant since slot {} with a balance of {} PANO",
                Emoji("✅ ", ""),
                slot,
...
fn main() {
    ...
    panoptes_metrics::set_panic_hook("stake-monitor");
