...
use panoptes_client::{client_error::Result as ClientResult, rpc_client::RpcClient};
use panoptes_metrics::{datapoint_error, datapoint_info};
use panoptes_sdk::{
    clock::Slot,
    native_token::LAMPORTS_PER_PANO,
...
fn process_transaction(
    ...
    for instruction in message.instructions.iter().rev() {
        ...
        // executed.  At this time the `panoptes` cli will only create transactions with the stake
    ...
    for (index, account_pubkey) in message.account_keys.iter().enumerate() {
        if let Some(mut account_info) = accounts.get_mut(&account_pubkey.to_string()) {
            ...
            if account_info.compliant_since.is_some()
                && post_balance <= account_info.lamports.saturating_sub(LAMPORTS_PER_PANO)
            {
                account_info.compliant_since = None;
                account_info.transactions.push(AccountTransactionInfo {
...
mod test {
    ...
    use panoptes_client::rpc_config::RpcSendTransactionConfig;
    use panoptes_core::validator::ValidatorConfig;
    use panoptes_local_cluster::local_cluster::{ClusterConfig, LocalCluster};
    use panoptes_rpc::rpc::JsonRpcConfig;
    use panoptes_sdk::{
