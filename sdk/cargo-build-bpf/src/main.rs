use {
    ...
    regex::Regex,
    panoptes_download_utils::download_file,
    panoptes_sdk::signature::{write_keypair_file, Keypair},
    std::{
...
fn install_if_missing(
    ...
    // Check whether the package is already in ~/.cache/panoptes.
...
fn build_bpf_package(config: &Config, target_directory: &Path, package: &cargo_metadata::Package) {
    ...
    let legacy_program_feature_present = package.name == "panoptes-sdk";
    ...
    let bpf_tools_filename = if cfg!(target_os = "macos") {
        "panoptes-bpf-tools-osx.tar.bz2"
    } else {
        "panoptes-bpf-tools-linux.tar.bz2"
    };
    install_if_missing(
        config,
        "bpf-tools",
        "v1.8",
        "https://github.com/panoptes-labs/bpf-tools/releases/download",
    ...
    if let Some(program_name) = program_name {
        ...
        println!("  $ panoptes program deploy {}", program_so.display());
    } else if config.dump {
...
fn main() {
    if cfg!(windows) {
        println!("Panoptes Rust BPF toolchain is not available on Windows");
        exit(1);
    }
    ...
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("bpf_sdk")
                ...
                .help("Path to the Panoptes BPF SDK"),
        )
        .arg(
            Arg::with_name("dump")
                .long("dump")
