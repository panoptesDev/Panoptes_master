//! @brief Panoptes builtin helper macros
    ...
    ($name:ident, $id:path, $entrypoint:expr) => {
        ...
        macro_rules! $name {
            () => {
                ...
                // see https://github.com/panoptes-labs/panoptes/issues/10933.
                // For now, we need to use `::panoptes_sdk`
                //
                // `respan!` respans the path `$crate::id`, which we then call (hence the extra
                // parens)
                (
                    stringify!($name).to_string(),
                    ::panoptes_sdk::respan!($crate::$id, $name)(),
                    $entrypoint,
                )
            };
        }
...
/// mod item_wrapper {
/// use panoptes_sdk::keyed_account::KeyedAccount;
/// use panoptes_sdk::instruction::InstructionError;
/// use panoptes_sdk::pubkey::Pubkey;
/// use panoptes_sdk::declare_builtin;
///
...
///
/// # use item_wrapper::id;
/// assert_eq!(id(), panoptes_sdk::system_program::ID);
/// ```
#[macro_export]
